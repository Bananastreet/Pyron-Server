...
public class NPCProcess {
    ...
    private void processing() {
        ...
        if (npc.getNpcId() == VoteBoss.bossID) {
            npc.getBehaviour().setRespawn(false);
        }

        if (npc.getNpcId() == Abomination.bossID) {
            npc.getBehaviour().setRespawn(false);
        }
            ...
            if (npc.actionTimer == 0 && !npc.applyDead && !npc.needRespawn) {
                ...
                if (killerIndex <= PlayerHandler.players.length - 1) {
                    ...
                    if (target != null) {
                        ...
                        ChallengeHandler.killChecker(target, npc.getNpcId(), false);
                    }
                }
                ...
                switch (npc.getNpcId()) {
                    ...
                    case Hespori.NPC_ID:
                        ...
                        break;
                    case 7310:
                        PlayerHandler.nonNullStream().filter(p -> Boundary.isIn(p, Boundary.NEW_WILDY_DUNGEON))
                                .forEach(p -> {
                                    if (p.brassicanDamage > 50) {
                                        p.getNpcDeathTracker().add(NpcDef.forId(7310).getName(), NpcDef.forId(7310).getCombatLevel(), 0);
                                        Server.getDropManager().create(p, npc, new Location3D(p.getX(), p.getY(), p.getHeight()), 1, 7310);
                                        p.brassicanDamage = 0;
                                    }
                                });
                        break;
                    case FragmentOfSeren.NPC_ID:
                        ...
                        FragmentOfSeren.rewardPlayers(npc);
