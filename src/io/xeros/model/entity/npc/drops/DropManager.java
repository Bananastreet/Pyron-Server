...
public class DropManager {
    ...
    public static boolean wearingZenithRing(Player player) {
        if (player.getItems().isWearingItem(25541) || player.getItems().isWearingItem(2951) || player.getItems().isWearingItem(2952) || player.getItems().isWearingItem(2953) || player.getItems().isWearingItem(2954)) {
            return true;
        } else {
            return false;
        }
    ...
    public void handleWildernessDungeonDrops(Player player, int npcId, int dropX, int dropY, int dropZ) {
        NpcDef npcDefinition = NpcDef.forId(npcId);
        String name = npcDefinition.getName().toLowerCase();
        boolean lowNPC = npcDefinition.getCombatLevel() > 0 && npcDefinition.getCombatLevel() <= 80;
        boolean mediumNPC = npcDefinition.getCombatLevel() > 80 && npcDefinition.getCombatLevel() <= 110;
        boolean highNPC = npcDefinition.getCombatLevel() > 110 && npcDefinition.getCombatLevel() <= 331;
        boolean superNPC = npcDefinition.getCombatLevel() > 331;
        int lowChance = 2000;
        int mediumChance = 1000;
        int highChance = 500;
        int superChance = 250;
        int odds = 25000;
        if (lowNPC) {
        ...
        int randomReward[] = {22415, 11740, 26706, 26706, 25365, 25087, 76};
        int reward = randomReward[(int) (Math.random()*randomReward.length)];

        if (Misc.random(odds) == 15) {
            player.sendMessage("@red@You have received a "+ ItemAssistant.getItemName(reward)+" (to your inventory, bank, or under you).");
    ...
    public static void dropPerkTicket(Player player, int npcId, int dropX, int dropY, int dropZ) {
        NpcDef npcDefinition = NpcDef.forId(npcId);
        String name = npcDefinition.getName().toLowerCase();
        boolean lowNPC = npcDefinition.getCombatLevel() > 0 && npcDefinition.getCombatLevel() <= 80;
        boolean mediumNPC = npcDefinition.getCombatLevel() > 80 && npcDefinition.getCombatLevel() <= 110;
        ...
        if (player.hasPerk(Player.PERKY_JERKY)) {
            odds *= 0.5;
        }

       // if (Misc.random(odds) == 15) {
          //  player.sendMessage("You have received a perk ticket (to your inventory, or under you)");
          //  PlayerHandler.executeGlobalMessage("@cr35@ @blu@[PERKS] @red@" + player.getDisplayName() + " has a received perk ticket from a " + Misc.capitalize(name) + "!");
    ...
    public static void dropCoinBag(Player player, int npcId, int dropX, int dropY, int dropZ) {
        ...
        if (Misc.random(8) == 5) {
            ...
            if (player.hasPerk(Player.COIN_COLLECTOR)) {
                player.getItems().addItemUnderAnyCircumstance(itemId, 2);
            } else {
    ...
    public static void dropResourcePack(Player player, int npcId, int dropX, int dropY, int dropZ) {
        ...
        if (npcDefinition != null && Misc.random(20) == 1) {
            ...
            int extraPack = 0;
            if (player.hasPerk(Player.RESOURCE_COLLECTOR)){
                player.getItems().addItemUnderAnyCircumstance(box, 2);
            } else {
    ...
    public List<GameItem> getDropSample(Player player, int npcId) {
        ...
        if (group.isPresent()) {
            double modifier = getActualDropModifier(player);
    ...
    public void handle(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        if (Boundary.isIn(npc, Boundary.JELLY_ROOM)) {
            return;
        }
        ...
        if (Boundary.isIn(player, Boundary.WILDY_SLAYER_CAVE)) {
            handleWildernessDungeonDrops(player, npcId, location.getX(), location.getY(), location.getZ());
        }
        ...
        if (Misc.random(crystalRandom) == 1) {
            ...
            if (player.hasPerk(Player.CRYSTAL_COLLECTOR)) {
                if (player.getItems().freeSlots() > 0) {
                        extraKey = 1;
                    player.getItems().addItemUnderAnyCircumstance(specialItemId, keyAmount + extraKey);
                } else {
    ...
    public void create(Player player, NPC npc, Location3D location, int repeats, int npcId) {
        ...
        group.ifPresent(g -> {
            double modifier = getActualDropModifier(player);
            if (BossSpotlight.active && BossSpotlight.isSpotlightNpc(npcId)) {
                modifier -= .05;
            }
            if (npcId == 11246 && modifier < 0.8) {
                modifier = 0.8;
            }
            List<GameItem> drops = g.access(player, npc, modifier, repeats, npcId);
            for (GameItem item : drops) {
                onDrop(player, item, npcId);
                if (item.getId() == 619 && player.seasonalScrollActive && player.hasPerk(Player.SEASONAL_PICKUP)) {
                ...
                if (item.getId() == 619 && player.hasPerk(Player.SEASONAL_PICKUP)) {
                    player.getItems().addItemUnderAnyCircumstance(item.getId(), item.getAmount());
                    //player.sendMessage("You've had" + item.getAmount() +"x fall 2023 tickets added to inventory or bank.");
                } else {
                    Server.itemHandler.createGroundItem(player, item.getId(), location.getX(), location.getY(),
                            location.getZ(), item.getAmount(), player.getIndex());
                }
    ...
    private double getActualDropModifier(Player player) {
        /*
           Example -= 0.10  == 10%
           Example -= 0.25  == 25%
        ...
        if (player.hasFollower && player.petSummonId == 13247 && Boundary.isIn(player, Boundary.CERBERUS_BOSSROOMS)) {
            modifier -= 0.10;
        }
        if (Boundary.isIn(player, Boundary.DIAMOND_ZONE)) {
            modifier -= 0.05;
        }
        if (Boundary.isIn(player, Boundary.ONYX_ZONE_NEW)) {
            modifier -= 0.06;
        }
        if (Boundary.isIn(player, Boundary.ZENYTE_ZONE)) {
            modifier -= 0.07;
        }
        ...
        if (player.getItems().isWearingItem(22941)) {
            modifier -= 0.04;
        }
        if (player.getItems().isWearingItem(3721)) {
            modifier -= 0.01;
        }
        if (player.getItems().isWearingItem(6201)) {
            modifier -= 0.01;
        }
        if (player.getItems().isWearingItem(2953)) {
            modifier -= 0.02;
        }
        if (player.getItems().isWearingItem(2954)) {
            modifier -= 0.03;
        }
        if (player.getItems().isWearingItem(2572)) {
            modifier -= .03;
        } else if (player.getItems().isWearingItem(12785) || wearingZenithRing(player)) {
            modifier -= player.hasPerk(Player.RING_OF_WEALTHIER) ? .10 : .08;
        }
        ...
        if (player.getItems().playerHasItem(30121) || (player.hasFollower && player.petSummonId == 30121)) {
            ...
            modifier -= .10;
        } else if (player.getItems().playerHasItem(30014) || (player.hasFollower && (player.petSummonId == 30014 || player.petSummonId == 30022))) {
            modifier -= .05;
        }
        ...
        if (player.hasFollower && player.petSummonId == 30111) {
            modifier -= .1;
        }
        ...
        double donatorModifier = DonatorRank.getDroprate(DonatorRank.getHighestRank(player));
        modifier -= donatorModifier * 0.1;
        //if (modifier < 0.5) modifier = 0.5;

        return modifier;
    }
    public static double getVisualDroprate(Player player) {

        /*
            Sets the value to be displayed when doing ::droprate command.
            Example: Modifier += 3 will show (Or add) 3%.
         */
        int modifier = 0;
        ...
        if (player.getItems().isWearingItem(10557)) { //collector icon
            modifier += 2;
        }
        if (player.getItems().isWearingItem(2953)) {
            modifier += 2;
        ...
        int donatorModifier = DonatorRank.getDroprate(DonatorRank.getHighestRank(player));
            modifier += donatorModifier;
        if (player.getItems().isWearingItem(2572)) {
            modifier += 3;
        } else if (player.getItems().isWearingItem(12785) || wearingZenithRing(player)) {
            modifier += player.hasPerk(Player.RING_OF_WEALTHIER) ? 10 : 8;
        }
        if (player.getItems().isWearingItem(22941)) {
            modifier += 4;
        }
    ...
    public void select(Player player, int button) {
        ...
        //Finding NPC ID
        if(player.dropInterfaceSearchList.isEmpty()){
            return;
        }
        if (player.dropInterfaceSearchList.size() > listIndex) {
    ...
    public void openForNpcId(Player player, int npcId) {
        ...
        group.ifPresent(g -> {
          ...
          /*  var npcLvl = npcDef.getCombatLevel();
            var getPerkTicketChance = npcLvl > 0 && npcLvl <= 80 ? 5000 : npcLvl > 80 && npcLvl <= 110 ? 3000 : npcLvl > 110 && npcLvl <= 331 ? 2000 : npcLvl > 331 ? 400 : 0;
           if (getPerkTicketChance > 0) {
                writeItem(player, Items.PERK_TICKET, 1, 1, "Rare", getPerkTicketChance);
            } */
    ...
    public void onDrop(Player player, GameItem item, int npcId) {
        ...
        if (Boundary.isIn(player, Boundary.REV_CAVE) && player.getItems().isWearingItem(Items.AMULET_OF_AVARICE)) {
            if (item.getDef().getNoteId() > 0 && !item.getDef().isNoted()) {
                item.changeDrop(item.getDef().getNoteId(), item.getAmount());
            }
        }
...
//		}
		if (item.getId() == 995 && player.collectCoins == true && (player.getItems().freeSlots() > 0 || player.getItems().playerHasItem(995))) {
			if ((player.getItems().isWearingItem(12785)) || (player.getItems().isWearingItem(2572)) || wearingZenithRing(player)) {
