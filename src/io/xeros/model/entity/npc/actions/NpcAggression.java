...
public class NpcAggression {
    ...
    public static void doAggression(NPC npc, NPCHandler npcHandler) {
        if ((isAggressive(npc) || npc.getBehaviour().isAggressive())
            ...
            for (Player player : PlayerHandler.players) {
                ...
                if (Boundary.isIn(player, Boundary.DIAMOND_ZONE) || Boundary.isIn(npc, Boundary.DIAMOND_ZONE)) {
                    return;
                }
                if (Boundary.isIn(player, Boundary.ONYX_ZONE_NEW) || Boundary.isIn(npc, Boundary.ONYX_ZONE_NEW)) {
                    return;
                }
                if (Boundary.isIn(player, Boundary.ZENYTE_ZONE) || Boundary.isIn(npc, Boundary.ZENYTE_ZONE)) {
                    return;
                }
                if (npc.getNpcId() == FragmentOfSeren.NPC_ID && !Boundary.isIn(player, Boundary.SEREN_ROOM)) {
                    return;
                }
            ...
            if (closestPlayer != null) {
                ...
                && npc.getDefinition().getCombatLevel() > 0
                && !npc.underAttack
                && !npc.isDead()
            ...
            if (System.currentTimeMillis() - npc.lastRandomlySelectedPlayer > 10000) {
                ...
                if (player > 0 && npc.getDefinition().getCombatLevel() > 0) {
    ...
    public static boolean isAggressive(NPC npc) {
        ...
        if (Boundary.isIn(npc, Boundary.RUBY_PLUS_ZONE) || Boundary.isIn(npc, Boundary.DIAMOND_ZONE) || Boundary.isIn(npc, Boundary.ONYX_ZONE_NEW) || Boundary.isIn(npc, Boundary.ZENYTE_ZONE)) {
            return false;
        }
