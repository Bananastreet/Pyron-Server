...
public class NPCHandler {
    ...
    public boolean switchesAttackers(NPC npc) {
        ...
        case 319:
            case 11246: //rev boss
            case 10532: //avatar of destruction
            case 7414: //vote boss
            case 3473: //inadequacy
            case 8262: //abomination
        case 7554:
        case 320: 
        case 5535: 
        //case 2551:
    ...
    public void followPlayer(NPC npc, int playerId) {
        ...
        if (npc.getNpcId() == 7414) { //vote boss
            return;
        }
        if (npc.getNpcId() == 3473) { //inadequacy
            return;
        }
        if (npc.getNpcId() == 8262) { //abomination
            return;
        }
        if (npc.getNpcId() == 10532) { //avatar of destruction
            return;
        }
    ...
    public void attackPlayer(Player c, NPC npc) {
        ...
        if (npc != null) {
            ...
            switch (npc.getNpcId()) {
                case inadequacy.bossID:
                    int inadATK = Misc.random(6);
                    if (inadATK == 1) {
                        npc.forceChat("RANGE!");
                        for (int j = 0; j < PlayerHandler.players.length; j++) {
                            if (PlayerHandler.players[j] != null) {
                                Player player = PlayerHandler.players[j];
                                if (Boundary.isIn(player, Boundary.INADEQUACY)) {
                                    player.sendMessage("@gre@The inadequacy is preparing for a range attack.");
                                    CycleEventHandler.getSingleton().addEvent(new Object(), new CycleEvent() {
                                        @Override
                                        public void execute(CycleEventContainer container) {
                                            if (player.prayerActive[CombatPrayer.PROTECT_FROM_RANGED]) {
                                                player.appendDamage(npc, Misc.random(10, 20), Hitmark.HIT);
                                                player.gfx100(302);
                                            } else {
                                                player.appendDamage(npc, Misc.random(40, 60), Hitmark.HIT);
                                                player.gfx100(302);
                                            }
                                            container.stop();
                                        }
                        ...
                        for (int j = 0; j < PlayerHandler.players.length; j++) {
                            if (PlayerHandler.players[j] != null) {
                                Player player = PlayerHandler.players[j];
                                if (Boundary.isIn(player, Boundary.INADEQUACY)) {
                                    player.sendMessage("@blu@The inadequacy is preparing for a mage attack.");
                                    CycleEventHandler.getSingleton().addEvent(new Object(), new CycleEvent() {
                                        @Override
                                        public void execute(CycleEventContainer container) {
                                            if (player.prayerActive[CombatPrayer.PROTECT_FROM_MAGIC]) {
                                                player.appendDamage(npc, Misc.random(10, 20), Hitmark.HIT);
                                                player.gfx100(1463);
                                            } else {
                                                player.appendDamage(npc, Misc.random(40, 60), Hitmark.HIT);
                        ...
                        for (int j = 0; j < PlayerHandler.players.length; j++) {
                            if (PlayerHandler.players[j] != null) {
                                Player player = PlayerHandler.players[j];
                                if (Boundary.isIn(player, Boundary.INADEQUACY)) {
                                    player.sendMessage("@red@The inadequacy is preparing for a melee attack.");
                                    CycleEventHandler.getSingleton().addEvent(new Object(), new CycleEvent() {
                                        @Override
                                        public void execute(CycleEventContainer container) {
                                            if (player.prayerActive[CombatPrayer.PROTECT_FROM_MELEE]) {
                                                player.appendDamage(npc, Misc.random(10, 20), Hitmark.HIT);
                                                player.gfx100(141);
                                            } else {
                                                player.appendDamage(npc, Misc.random(40, 60), Hitmark.HIT);
                ...
                case 10532:
                    if (Misc.random(6) == 1) {
                        for (int j = 0; j < PlayerHandler.players.length; j++) {
                            if (PlayerHandler.players[j] != null) {
                                Player player = PlayerHandler.players[j];
                                if (Boundary.isIn(player, Boundary.AVATAR_OF_DESTRUCTION)) {
                                    player.appendDamage(npc, Misc.random(15, 35), Hitmark.HIT);
                                    player.sendMessage("@red@The avatar damages you through the floor.");
                                }
                            }
                        }
                    }
    ...
    public int getMaxHit(Player c, NPC npc) {
        ...
        if (npc.getNpcId() == FragmentOfSeren.NPC_ID) {
