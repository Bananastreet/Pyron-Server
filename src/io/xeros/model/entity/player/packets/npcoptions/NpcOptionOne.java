...
public class NpcOptionOne {
	...
	public static void handleOption(Player player, int npcType) {
		...
		switch (npcType) {
			case 4589:
				player.getShops().openShop(363);
				break;
			case 3308:
				if (Configuration.leaderboardEnabled) {
					player.leaderboardManager.open();
				} else {
					player.sendMessage("The leaderboard is currently disabled.");
				}
				break;
			case 3925:
				player.sendMessage("The Pyron 2024 battle pass ends on January 31st, 2024.");
				player.getPA().showInterface(44000);
				break;
			case 9125:
		...
		case 9011:
			...
			player.sendMessage("Vote at @blu@::vote@bla@, then claim with @blu@::claimvote@bla@ to earn vote crystals.");
			break;
		case 1011:
		...
		case 6797: // Nieve
			...
			break;
			case 6785: // hosa
				player.start(new DialogueBuilder(player).setNpcId(11278).option(new DialogueOption("Buy boss challenge @red@(10M)", p -> {
					ChallengeHandler.assignChallenge(p);
					p.getPA().closeAllWindows();
				}), new DialogueOption("Cancel boss challenge @red@(5m)", plr -> {
					ChallengeHandler.cancelChallenge(plr);
					plr.getPA().closeAllWindows();
				}), new DialogueOption("Information", plr -> {
					plr.sendMessage("Bosses challenges will assign you 5 random bosses to kill.");
					plr.sendMessage("Different bosses have different amounts you'll be assigned to kill.");
					plr.sendMessage("Once you've completed the challenge, you'll receive a boss challenge box.");
					plr.getPA().closeAllWindows();
				}), new DialogueOption("Nevermind", plr -> {
					plr.getPA().closeAllWindows();
				})));
				break;
		case 308:
