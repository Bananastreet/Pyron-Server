...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		switch (objectType) {
			case 40388:
				c.getPA().movePlayer(3385, 10052, 0);
				break;
			case 40389:
					c.getPA().movePlayer(3260, 3663, 0);
				break;
			case 40390:
			...
			case 33016:
				...
				c.objectXOffset = 3;
				c.objectYOffset = 3;
				new CorruptedSlayerKey().roll(c);
				break;
			case 43478:
				if (Boundary.isIn(c, Boundary.SEREN_ROOM)) {
					c.getPA().movePlayer(3160, 5357, 0);
				} else {
                    if (!FragmentOfSeren.isActive) {
						FragmentOfSeren.isActive = true;
						MonsterHunt.Npcs randomNpc = MonsterHunt.Npcs.FRAGMENT_OF_SEREN;
						FragmentOfSeren.currentSeren = NPCSpawning.spawnNpcOld(FragmentOfSeren.NPC_ID, 3163, 5334, 0, 1, 1500, randomNpc.getMaxHit(), randomNpc.getAttack(), randomNpc.getDefence()/*, false*/);
					}
					c.getPA().movePlayer(3164, 5344, 0);
				}
				break;
			case 4470:
					if (c.getY() == 2757) {
						if (c.getX() == 1246 || c.getX() == 1247 || c.getX() == 1248 || c.getX() == 1249) { //entrance
							c.getPA().movePlayer(c.getX(), c.getY() + 1, c.getHeight());
						}
						if (c.getX() == 1266 || c.getX() == 1267 || c.getX() == 1268 || c.getX() == 1269) { //graardor entrance
							c.getPA().movePlayer(c.getX(), c.getY() + 1, c.getHeight());
						}
					}
					if (c.getY() == 2758) {
					...
					if (c.getY() == 2763) {
						if (c.getX() == 1266 || c.getX() == 1267 || c.getX() == 1268 || c.getX() == 1269) { //kalphite entrance
							c.getPA().movePlayer(c.getX(), c.getY() + 1, c.getHeight());
						}
							if (c.getX() == 1246 || c.getX() == 1247 || c.getX() == 1248 || c.getX() == 1249) { //kree and zily entrance
								c.getPA().movePlayer(c.getX(), c.getY() + 1, c.getHeight());
							}
					}
					if (c.getY() == 2764) {
				...
				break;
			case 21316:
					c.getPA().movePlayer(2385, 10258, 1);
				break;
			case 21317:
				c.getPA().movePlayer(2385, 10264, 1);
				break;
			case 36087://hunllef reward chest
				if (c.getItems().playerHasItem(23776)) {
					NewHunllefKey.lootKey(c);
				} else {
					c.sendMessage("You need a key to open this chest.");
				}
				break;
			case 37340: //hunllef start
				if (!c.getPermAttributes().getOrDefault(Player.HUNLLEF_WARNING)) {
					Hunllef.start(c);
					return;
				}
							...
							if (player.getDisplayName().equalsIgnoreCase(str)) {
								if (player.getInstance() != null && !player.getInstance().isDisposed()) {
									/*for (NPC npc : player.getInstance().getNpcs()) {
										if (npc.getNpcId() == 11278 && npc.getHealth().getCurrentHealth() < (npc.getHealth().getMaximumHealth() - 100))  {
			...
			case 1521:
				...
				break;
			case 36197:
				c.getPA().startTeleport(3095, 3507, 0, "modern", true);
				break;
			case 34727:
				c.getPA().startTeleport(3095, 3507, 0, "modern", true);
				break;
...
//					} else {
			...
			case 36691:
				c.objectDistance = 3;
				c.getPA().movePlayer(2864, 4955, 0);
				break;
			case 36692:
				c.objectDistance = 3;
			...
			case 17385:
				if (Boundary.isIn(c, Boundary.EDGE_DUNG_LADDER)) {
					...
					AgilityHandler.delayEmote(c, "CLIMB_UP", 3101, 3472, 0, 2);
			...
			case 10:
				if (!c.getRights().isOrInherits(Right.DIAMOND_DONATOR)) {
			...
			case 7479:
				...
				break;
			case 11803:
				if (c.getRights().isOrInherits(Right.ONYX_DONATOR)) {
