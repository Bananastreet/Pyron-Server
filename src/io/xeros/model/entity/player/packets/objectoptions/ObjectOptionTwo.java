...
public class ObjectOptionTwo {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		...
		switch (objectType) {
			case 30720://ambiguous chest
				c.start(new DialogueBuilder(c).option(new DialogueOption("Chest Keys", p -> {
					LootableInterface.openSpecificTable(c, LootableInterface.LootableView.CHEST_KEY);
				}), new DialogueOption("Wilderness Keys", plr -> {
					LootableInterface.openSpecificTable(c, LootableInterface.LootableView.WILDERNESS_KEY);
				})));
				break;
		...
		case 34660:
		case 34662:
			LootableInterface.openSpecificTable(c, LootableInterface.LootableView.BRIMSTONE_KEY);
		...
		case 16700:
		case 10583:
		case 10355://warriors guild bank
			c.inBank = true;
			c.getPA().c.itemAssistant.openUpBank();
			break;
