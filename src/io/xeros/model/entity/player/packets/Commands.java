...
//        }
            ...
            if (playerCommand.equals("printdrdata")) {
                Right rank = DonatorRank.getHighestRank(c);
                int droprate = DonatorRank.getDroprate(rank);
                double actualDroprate = (droprate * 0.01);
                c.sendMessage("Rank: "  +rank.toString()+ " DR:" + droprate);
                c.sendMessage("Actual DR: " + actualDroprate);
                System.out.print("DR Modifier: " + actualDroprate);
            }
            ...
            if (playerCommand.equals("rights")) {
                ...
                c.sendMessage("isExreme_Donor: " + c.getRights().contains(Right.EMERALD_DONATOR));
            ...
            if (playerCommand.startsWith("bp")) {
                BattlepassHandler.openBattlepass(c);
            }

            if (playerCommand.startsWith("addbpxp")) {
                BattlepassLevels.addBattlepassExperience(c, 1000);
            }

            if (playerCommand.startsWith("foe") && c.getRights().isOrInherits(Right.PLAYER)) {
            ...
            if (playerCommand.equals("tob")) {
                c.moveTo(new Position(3671, 3219, 0));
                new TobParty().add(c);
                c.getTobContainer().startTob();
            }
            if (playerCommand.equals("tobnext")) {
                if (c.getTobContainer().inTob()) {
                    Lists.newArrayList(c.getInstance().getNpcs()).forEach(NPC::unregisterInstant);
                    TobInstance instance = (TobInstance) c.getInstance();
    ...
    private void handleClanMessaging(Player player, String query) {
        ...
        if (player.clan != null) {
            if (Configuration.ccLogsEnabled) {
                DiscordWebhook webhook = new DiscordWebhook(Configuration.ccLogs);
                webhook.addEmbed(new DiscordWebhook.EmbedObject()
                        .setTitle(player.clan.getFounder() + " Clan Chat")
                        .setDescription(player.getDisplayName() + ": " + query.substring(1))
                        .setColor(Color.RED));
                try {
