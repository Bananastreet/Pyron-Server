...
public class ClickingButtonsNew implements PacketType {
    ...
    public void processPacket(Player c, int packetType, int packetSize) {
        ...
        c.tierManager.clickButton(buttonId);
        c.leaderboardManager.clickButton(buttonId);
        ...
        if (buttonId == 51904) { //view details world event button in quest tab
            if (BossSpotlight.active) {
                c.sendMessage("@blu@{} currently on spotlight.", BossSpotlight.selectedBoss.getNpcName());
            }
        }
