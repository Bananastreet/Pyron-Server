...
public class ItemOptionTwo implements PacketType {
	...
	public void processPacket(Player player, int packetType, int packetSize) {
		...
		switch (itemId) {
			case 6119:
				if (player.getItems().playerHasItem(6119) && player.extraBankSpace < 1250) {
					player.getItems().deleteItem(6119, 1);
					player.extraBankSpace += 50;
					player.sendMessage("You now have "+ player.getBank().getBankCapacity() + " banking capacity.");
				} else {
					player.sendMessage("You can not redeem any additional stones.");
				}
				break;
			case 12885:
			case 13277:
