...
public class WearItem implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		if (wearId == Items.AMULET_OF_AVARICE) {
			c.start(new DialogueBuilder(c).statement("Wearing the Amulet of Avarice will cause you to be skulled.", "Are you sure?")
					.option(new DialogueOption("Yes", player -> {
						player.isSkulled = true;
						player.skullTimer = Configuration.SKULL_TIMER;
						player.headIconPk = 0;
						player.getPlayerAssistant().resetFollow();
						player.attacking.reset();
						player.getItems().equipItem(Items.AMULET_OF_AVARICE, player.wearSlot);
						player.getPA().closeAllWindows();

					}),
					new DialogueOption("Never mind", plr -> {
