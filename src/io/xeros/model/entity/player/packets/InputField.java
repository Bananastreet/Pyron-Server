		...
		case 59527:
			...
			if (HelpDatabase.getDatabase().requestable(player)) {
				...
				if (staff.size() > 0) {
					...
					DiscordWebhook webhook = new DiscordWebhook(Configuration.helpreq);
					webhook.addEmbed(new DiscordWebhook.EmbedObject()
							.setTitle(player.getDisplayName())
							.setDescription("[HelpDB] " + player.getDisplayName() + "" + " is requesting help, type ::helpdb to view their request.")
							.setColor(Color.RED));
					try {
