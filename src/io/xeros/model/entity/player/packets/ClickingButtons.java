...
import io.xeros.content.battlepass.BattlepassHandler;
...
import io.xeros.content.commands.all.sigils;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if (BattlepassHandler.handleButton(c, actionButtonId)) {
			return;
		}
		if (c.getUpgradeHandler().selectTab(actionButtonId) || c.getUpgradeHandler().button(actionButtonId)) {
			return;
		}
		...
		if (PerkHandler.handleButtons(c, actionButtonId)) {
			return;
		}
		if (c.getCollectionLog().handleActionButtons(c, actionButtonId)) {
			return;
		}
		if (sigils.buttons(c, actionButtonId)) {
			return;
		}
		...
		if (actionButtonId == 171235) {
			int hweenMask[] = {1053, 1055, 1057, 3700, 3706, 3707, 3709, 3708, 3710, 11847, 7811};


			if (c.getPermAttributes().getOrDefault(Player.BATTLEPASS_WINTER_2023)) {
				c.sendMessage("You've already purchased this battle pass.");
				return;
			}
			if (c.getItems().freeSlots() < 10) {
				c.sendMessage("You need 10 free inventory slots before purchasing the battle pass.");
				return;
			}
			if (c.getItems().playerHasItem(7478, 25) && !c.getPermAttributes().getOrDefault(Player.BATTLEPASS_WINTER_2023)) {
				c.getItems().deleteItem(7478, 25);
				DonatorRank.addTokensSpent(c, 25);
				c.getItems().addItem(23497, 5);
				c.getItems().addItem(11740, 1);
				c.getItems().addItem(26706, 1);
				c.getItems().addItem(25365, 1);
				c.getItems().addItem(25087, 1);
				c.getItems().addItemUnderAnyCircumstance(hweenMask[(int) (Math.random()*hweenMask.length)], 1);
				c.getPermAttributes().put(Player.BATTLEPASS_WINTER_2023, true);
				c.sendMessage("You've purchased the battle pass for 25 donator tokens.");
				c.getPA().removeAllWindows();
				return;
			} else {
				c.sendMessage("You need 25 donator tokens to purchase the battle pass.");
				return;
			}
		}
		...
		switch (actionButtonId) {
			...
			case 183156:
				...
				if (!(c.getSuperMysteryBox().canMysteryBox) || !(c.getNormalMysteryBox().canMysteryBox) ||
						!(c.getUltraMysteryBox().canMysteryBox) || !(c.getFoeMysteryBox().canMysteryBox) || !(c.getPetMysteryBox().canMysteryBox) ||
						!(c.getYoutubeMysteryBox().canMysteryBox) || !(c.getPVPMysteryBox().canMysteryBox) || !(c.getRaresMysteryBox().canMysteryBox) || !(c.getGodwarsMysteryBox().canMysteryBox)
				) {
				...
				switch(c.boxCurrentlyUsing) {
					...
					case 13346: //ultra rare
						...
						break;
					case 10560:
						c.getPVPMysteryBox().spin();
						break;
					case 10561:
						c.getRaresMysteryBox().spin();
						break;
					case 10563:
						c.getGodwarsMysteryBox().spin();
						break;
					case 6199:
			...
			case 183155:
				if (!(c.getSuperMysteryBox().canMysteryBox) || !(c.getNormalMysteryBox().canMysteryBox) || !(c.getUltraMysteryBox().canMysteryBox) || !(c.getFoeMysteryBox().canMysteryBox) || !(c.getPetMysteryBox().canMysteryBox)
						|| !(c.getGodwarsMysteryBox().canMysteryBox) || !(c.getYoutubeMysteryBox().canMysteryBox) || !(c.getPVPMysteryBox().canMysteryBox) || !(c.getRaresMysteryBox().canMysteryBox)) {
					c.getPA().showInterface(47000);
					c.sendMessage("@red@[WARNING] @blu@Please do not interrupt or you @red@WILL@blu@ lose items! @red@NO REFUNDS");
					break;
				}
...
/*			  if (Server.getMultiplayerSessionListener().inAnySession(c)) { return; }
			...
			case 75010:
				...
				c.getPA().spellTeleport(3095, 3507, 0, true);
