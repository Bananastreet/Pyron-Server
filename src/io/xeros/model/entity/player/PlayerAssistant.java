...
//		}
	...
	public void assembleSlayerHelmet() {
		...
		var requiredItems = new int[]{4166, 4168, 4164, 4551, 8901, 4155};
		var hasItems = Arrays.stream(requiredItems).allMatch(c.getItems()::playerHasItem);
		if (hasItems) {
			c.sendMessage("@blu@You assemble the pieces and create a Slayer helmet!");
			Arrays.stream(requiredItems).forEach(i -> c.getItems().deleteItem(i, 1));
			c.getItems().addItem(11864, 1);
		} else {
			c.getDH().sendStatement("You need a facemask, nose peg, spiny helmet, earmuffs, black mask,", "and enchanted gem to create a Slayer helmet.");
		}
	}
...
//	updateConfig(c, spell);
	...
	public void sendParent(int componentID, int child, int replace) {
		if (c.getOutStream() != null) {
			c.getOutStream().createFrame(190);
			c.getOutStream().writeDWord(componentID);
			c.getOutStream().writeDWord(child);
			c.getOutStream().writeDWord(replace);
			c.flushOutStream();
		}
	}

	public void sendSprite(int componentID, int normal, int hover) {
		c.getPA().sendString(-345,"sendSprite [" + componentID + "," + normal + "," + hover + "]");
	}
	...
	public void createProjectile(int x, int y, int offX, int offY, int angle, int speed, int gfxMoving,
		...
		// synchronized(c) {
		//System.out.println("SEND PROJEDCTILE PRE 1");
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrame(85);
			//System.out.println("SEND PROJEDCTILE 1");
	...
	public void createProjectile(int x, int y, int offX, int offY, int angle, int scale, int pitch, int speed, int gfxMoving,
								 int startHeight, int endHeight, int lockon, int time) {
		// synchronized(c) {
		//System.out.println("SEND PROJEDCTILE PRE 2");
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrame(85);
			//System.out.println("SEND PROJEDCTILE 2");
			c.getOutStream().writeByteC((y - (c.getMapRegionY() * 8)) - 2);
			c.getOutStream().writeByteC((x - (c.getMapRegionX() * 8)) - 3);
			c.getOutStream().createFrame(117);
			c.getOutStream().writeByte(angle);
...
//				43, 43, 0, 0);
		//System.out.println("SEND PROJEDCTILE PRE 3");
		if (c.getOutStream() != null) {
			c.getOutStream().createFrame(85);
			//System.out.println("SEND PROJEDCTILE 3");
	...
	private void createProjectile2(int x, int y, int offX, int offY, int angle, int speed, int gfxMoving,
		...
		// synchronized(c) {
		//System.out.println("SEND PROJEDCTILE PRE 4");
		if (c.getOutStream() != null && c != null) {
			c.getOutStream().createFrame(85);
			//System.out.println("SEND PROJEDCTILE 4");
			c.getOutStream().writeByteC((y - (c.getMapRegionY() * 8)) - 2);
			c.getOutStream().writeByteC((x - (c.getMapRegionX() * 8)) - 3);
			c.getOutStream().createFrame(117);
			c.getOutStream().writeByte(angle);
	...
	public boolean morphPermissions() {
		...
		if (!Boundary.isIn(c, Boundary.EDGEVILLE_PERIMETER)) {
			c.sendMessage("You need to be in Edgeville to do this.");
			return false;
		}
		if (Boundary.isIn(c, Boundary.OUTLAST_HUT)) {
			return false;
	...
	public boolean canTeleport(String teleportType) {
		...
		if (c.jailEnd > 1 && !c.getDisplayName().equalsIgnoreCase("bubly")) {
	...
	public void levelUp(int skill) {
		...
		if (c.getRights().isNotAdmin()) {
			if (getLevelForXP(c.playerXP[skill]) == 99) {
				...
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has reached level 99 <icon=" + iconId +"> " + s.toString() + " on " + c.getMode().getType().getFormattedName() + " mode!");
			}
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
		...
		if (c.getItems().playerHasItem(26032, 1) || c.getItems().playerHasItem(26146, 1)) {
			amount *= 1.15;
		}

		if (c.getItems().isWearingItem(24387)) {
			amount *= 1.10;
		}

		if (c.tomeOfExperienceActive) {
			amount *= 2;
		}

		if (c.getItems().isWearingItem(22948)) { //blessing of experience
			amount *= 1.15;
		}

		if (c.getItems().isWearingItem(24389)) {
			amount *= 1.16;
		}

		if (c.getItems().isWearingItem(24391)) {
			amount *= 1.16;
		}

		if (c.hasFollower) {
			if (c.petSummonId == 23939) {
				amount *= 1.25;
			}
			if (c.petSummonId == 30123) {
				amount *= 1.5;
			}
		}

		if (c.trinketOfExperienceActive) {
			amount *= 1.25;
		}

		if (c.getItems().isWearingItem(24393)) {
			amount *= 1.10;
		}
		...
		if (c.hasPerk(Player.GRIND_HATER)) {
			amount *= 1.1;
		}
		...
		boolean inCombat = c.underAttackByNpc > 0;
		if (!c.isIdle && !inCombat && !c.isOnAfkResource) {
			BattlepassLevels.addBattlepassExperience(c,Misc.random(1,2));
		}
		...
		if (oldExperience < Skill.MAX_EXP && oldExperience + amount >= Skill.MAX_EXP) {
			...
			c.xpMaxSkills += 1;
			if (c.getRights().isNotAdmin()) {
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has reached 200M XP in " +
				...
				if (c.xpMaxSkills > 21) {
					PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has reached 200M XP in all skills on " + c.getMode().getType().getFormattedName() + " mode!");
				}
			}
		}
		...
		for (var s : Skill.values()) {
			if (s == null) continue;
			var skillName = s.toString().toLowerCase();
			RandomChestKey.handleChance(c, skillName);
		}
		SeasonalTickets.giveTickets(Skill.forId(skill), c);
	...
	public static void decantResource(Player player, int item) {
		...
		if (!player.getRights().isOrInherits(Right.SAPPHIRE_DONATOR) && !player.getRechargeItems().hasItem(13111)) {
		...
		if (!player.getRights().isOrInherits(Right.SAPPHIRE_DONATOR)) {
