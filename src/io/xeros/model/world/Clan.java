...
public class Clan {
	...
	public void updateDisplayName(Player player) {
		if (activeMembers.removeIf(it -> it.getLoginName().equalsIgnoreCase(player.getLoginName()))) {
			activeMembers.add(new ClanMember(player.getLoginName(), player.getDisplayName(), player.getDisplayNameFormatted()));
			sort();
			setDirty(true);
		}
	}
	...
	public void updateInterface(Player player) {
		...
		for (int index = 0; index < 100; index++) {
			if (index < activeMembers.size()) {
				player.getPA().sendFrame126("<clan=" + getRank(activeMembers.get(index).getLoginName()) + ">" + activeMembers.get(index).getFormattedName(), 18144 + index);
			} else {
