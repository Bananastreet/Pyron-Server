...
import io.xeros.content.DonatorRank;
...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		if (c.myShopId == 44) {
			c.sendMessage(itemName + ": currently costs " + Misc.insertCommas(getSpecialItemValue(removeId)) + " slayer points.");
			return;
		}
		if (c.myShopId == 13) {
		...
		if (c.myShopId == 77) {
			c.sendMessage(itemName + ": currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] Vote crystals.");
			return;
		}
		if (c.myShopId == 199) {
			c.sendMessage(itemName + ": currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] AFK Tokens.");
			return;
		}
		if (c.myShopId == 198) {
			c.sendMessage(itemName + ": currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] Upgrade Tokens.");
			return;
		}
		if (c.myShopId == 80) {
		...
		if (c.myShopId == 171) {
			c.sendMessage(itemName + "@bla@: currently costs [ @pur@" + Misc.insertCommas(getSpecialItemValue(removeId)) + " @bla@] Exchange Tickets.");
			return;
		}
		if (c.myShopId == 172 || c.myShopId == 173) {
			c.sendMessage(itemName + ": currently exchanges for  [ @pur@" + getSpecialItemValue(removeId) + " @bla@] Exchange Tickets.");
			return;
		}
		if (c.myShopId == 131) {
		...
		if (c.myShopId == 9) {
			c.sendMessage(itemName + ": currently costs " + Misc.insertCommas(getSpecialItemValue(removeId)) + " Donator Tokens.");
			return;
		}
		if (c.myShopId == 60) {
			c.sendMessage(itemName + ": currently costs " + getExchangeItemValue(removeId, "buyFromShop") + " Exchange Tickets.");
			return;
		}
		if (c.myShopId == 110) {
			c.sendMessage(itemName + ": currently costs " + getSpecialItemValue(removeId) + " Stardust.");
			return;
		}
		if (c.myShopId == 18) {
		...
		if (c.myShopId == 363) {
			c.sendMessage(itemName + ": currently costs " + Misc.insertCommas(getSpecialItemValue(removeId)) + " fall 2023 tickets.");
			return;
		}
		if (c.myShopId == 115) {
	...
	public static int getExchangeItemValue(int id, String type) {
		ShopDef shopDef = type.equals("buyFromShop") ? ShopDef.getDefinitions().get(60) : ShopDef.getDefinitions().get(61);
		if (ItemDef.forId(id).isNoted()) {
			id = ItemDef.forId(id).getUnNotedIdIfNoted();
		}
	...
	public int getSpecialItemValue(int id) {
		...
		switch (c.myShopId) {
			case 110: //Stardust Shop
			case 9: //Donator Token Shop
		...
		case 15:
			...
			case 120:
				switch(id) {
					case 23947:
						return 50;
					case 24387: //experience hat
						return 35;
					case 24389: //experience coat
						return 50;
					case 24391: //experience trousers
						return 50;
					case 24393: //experience boots
						return 35;
					case 3722: //overloaded heart
						return 75;
					case 26482: //abyssal whip (or)
						return 35;
					case 26484: //abyssal tentacle (or)
						return 70;
					case 12019: //unlimited coal bag
						return 50;
	...
	public void sellToShopPrice(int removeId, int removeSlot) {
		...
		if (IsIn == false) {
			...
			if (c.myShopId == 44) {
				...
				var cost = getExchangeItemValue(removeId, "sellToShop");
				if (cost == Integer.MAX_VALUE) {
					c.sendMessage("You cannot sell this item to this shop.");
					return;
				}
				int origValue = getExchangeItemValue(removeId, "sellToShop");
				if (c.hasPerk(Player.EXCHANGE_MASTER)) {
					origValue *= 1.1;
				}
				c.sendMessage(ItemAssistant.getItemName(removeId) + ": shop will buy for " + origValue + " Exchange Tickets");
				...
				if (c.hasPerk(Player.PRO_SALESMAN)) {
					ShopValue *= 0.837;
				} else {
					ShopValue *= 0.537;
				}
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0 && itemID == (c.playerItems[fromSlot] - 1)) {
			...
			if (c.myShopId == 60) {
				TotPrice4 = getExchangeItemValue(itemID, "sellToShop");
				if (TotPrice4 == Integer.MAX_VALUE || TotPrice4 == 0) {
					c.sendMessage("You can't sell this item to this store.");
					return false;
				}
			}
			...
			if (c.getItems().freeSlots() > 0 || c.getItems().playerHasItem(995)) {
				if ((ItemDef.forId(itemID).isStackable() || ItemDef.forId(itemID).isNoted()) && c.getItems().playerHasItem(itemID, amount)) {
					...
					if (c.myShopId != 12 && c.myShopId != 29 && c.myShopId != 60 && c.myShopId != 44 && c.myShopId != 18  && c.myShopId != 83 && c.myShopId != 116 && c.myShopId != 115) {
						c.getItems().addItem(995, TotPrice2);
						logShop("received", 995, TotPrice2);
					} else if (c.myShopId == 60) {
						c.getItems().addItem(Items.CASTLE_WARS_TICKET, c.hasPerk(Player.EXCHANGE_MASTER) ? TotPrice4 *= 1.1 : TotPrice4);
						logShop("received", Items.CASTLE_WARS_TICKET, TotPrice4);
					} else if (c.myShopId == 29) {
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (!c.getMode().isItemPurchasable(c.myShopId, itemID) && c.myShopId != 198 && c.myShopId == 199) {
		...
		if (amount > 0) {
			...
			if(c.myShopId==124 && c.tokensSpent >=150 && itemID == 299){
				TotPrice2=0;
			}
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 60) {
			...
			int itemValue = getExchangeItemValue(itemID, "buyFromShop") * amount;
			if (!c.getItems().playerHasItem(Items.CASTLE_WARS_TICKET, itemValue)) {
				c.sendMessage("You do not have enough Exchange Tickets to purchase this.");
				return;
			}
			if (itemValue == 0) { return; } //fail-safe in case a price isn't added to an item
			c.getItems().deleteItem(Items.CASTLE_WARS_TICKET, itemValue);
			c.getItems().addItem(itemID, amount);
			c.getItems().sendInventoryInterface(3823);
			logShop("bought", itemID, amount);
			return;
		...
		if (c.myShopId == 44) {
			...
			int currency = Items.DONATOR_TOKEN;
			int cost = getSpecialItemValue(itemID) * amount;
			if (c.getItems().playerHasItem(currency,cost)) {
				if (c.getItems().freeSlots() > 0) {
					c.getItems().deleteItem(currency,cost);
					c.getItems().addItem(itemID, amount);
					DonatorRank.addTokensSpent(c, cost);
					c.sendMessage("You have now spent "+ c.tokensSpent +" donator tokens.");
					DiscordWebhook webhook = new DiscordWebhook(Configuration.donationStore);
					webhook.addEmbed(new DiscordWebhook.EmbedObject()
							.setTitle("Donator token shop")
							.setDescription(c.getDisplayName() + " has purchased "+ amount +"x "+ ItemAssistant.getItemName(itemID) +" for "+cost+ " donator tokens.")
							.setColor(Color.RED));
					try {
			...
			} //this makes me wish i didnt update my intellij :( lol is it cancer?
//		} else if (c.myShopId == 9) {
//
//			var costOfItems = getSpecialItemValue(itemID) * amount;
//			if (c.getItems().playerHasItem(Items.DONATOR_TOKEN, costOfItems)) {
//				if (c.getItems().freeSlots() > amount) {
//					DonatorRank.addTokensSpent(c, costOfItems);
//					c.getItems().addItem(itemID, amount);
//					c.getItems().deleteItem(Items.DONATOR_TOKEN, costOfItems);
//					c.getItems().sendInventoryInterface(3823);
//					logShop("bought", itemID, amount);
//					c.sendMessage("You have now spent "+ c.tokensSpent +" donator tokens.");
//					DiscordWebhook webhook = new DiscordWebhook(Configuration.donationStore);
//					webhook.addEmbed(new DiscordWebhook.EmbedObject()
//							.setTitle("Donator token shop")
//							.setDescription(c.getDisplayName() + " has purchased "+ amount +"x "+ ItemAssistant.getItemName(itemID) +" for "+costOfItems+ " donator tokens.")
//							.setColor(Color.RED));
//					try {
//						if (Configuration.DiscordEnabled)
//							webhook.execute(); //Handle exception
...
//					}
//				} else {
//					c.sendMessage("You do not have enough inventory space to buy this item.");
//				}
//			} else {
//				c.sendMessage("You do not have enough donator tokens to buy this item.");
//			}
		} else if (c.myShopId == 110) {
			var costOfItems = getSpecialItemValue(itemID) * amount;
			if (c.getItems().playerHasItem(Items.STARDUST, costOfItems)) {
				if (c.getItems().freeSlots() > amount) {
					c.getItems().addItem(itemID, amount);
					c.getItems().deleteItem(Items.STARDUST, costOfItems);
					c.getItems().sendInventoryInterface(3823);
					logShop("bought", itemID, amount);
				} else {
					c.sendMessage("You do not have enough inventory space to buy this item.");
				...
				c.sendMessage("You do not have enough Stardust to buy this item.");
			}
		...
		} else if (c.myShopId == 198) {
			var cost = getSpecialItemValue(itemID) * amount;
			if (c.getItems().playerHasItem(23497, cost)) {
				if (c.getItems().freeSlots() > amount) {
					c.getItems().deleteItem(23497, cost);
					c.getItems().addItem(itemID, amount);
					c.getItems().sendInventoryInterface(3823);
					logShop("bought", itemID, amount);
				} else {
					c.sendMessage("You do not have enough inventory space to buy this many.");
				}
			} else {
				c.sendMessage("You do not have enough upgrade tokens to buy this item.");
			}
		} else if (c.myShopId == 199) {
			var cost = getSpecialItemValue(itemID) * amount;
			if (c.getItems().playerHasItem(7774, cost)) {
				if (c.getItems().freeSlots() > amount) {
					c.getItems().deleteItem(7774, cost);
					c.getItems().addItem(itemID, amount);
					c.getItems().sendInventoryInterface(3823);
					logShop("bought", itemID, amount);
				} else {
				...
				c.sendMessage("You do not have enough afk tokens to buy this item.");
			}
		} else if (c.myShopId == 77) {
			var cost = getSpecialItemValue(itemID) * amount;
			if (c.getItems().playerHasItem(Items.VOTE_CRYSTAL, cost)) {
				if (c.getItems().freeSlots() > amount) {
					c.getItems().deleteItem(Items.VOTE_CRYSTAL, cost);
					c.getItems().addItem(itemID, amount);
					c.getItems().sendInventoryInterface(3823);
					logShop("bought", itemID, amount);
				} else {
				...
				c.sendMessage("You do not have enough vote crystals to buy this item.");
			}
			...
			if (c.exchangePoints >= getSpecialItemValue(itemID) * amount) {
				...
				c.sendMessage("You do not have enough Exchange Tickets to buy this item.");
			}
		} else if (c.myShopId == 172 || c.myShopId == 173) {
