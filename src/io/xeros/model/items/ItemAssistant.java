...
public class ItemAssistant {
			...
			if (ammoId != -1) {
				...
				if (ammoIsRangeAmmo) {
					if (player.getCombatItems().usingCrystalBow()
							...
							|| player.getCombatItems().usingFaerdhinen()
							|| weaponIsAmmo) {
	...
	public boolean equipItem(int wearID, int slot) {
		...
		if (player.playerItems[slot] == (wearID + 1)) {
			...
			if (targetSlot == -1) {
				if (wearID >= 5509 && wearID <= 5512 || wearID == 26784 || wearID == 21347 || wearID == 15098 || wearID == 11918 || wearID == 13656 || wearID == 7959 || wearID == 7960) {
					return false;
				} else {
	...
	public void deleteArrow() {
		...
		for (int itemId : Configuration.blessings) {
			if (player.getItems().isWearingItem(itemId)) {
				return;
			}
		}
		for (int itemId : RangeData.NO_ARROW_DROP) {
			if (player.getItems().isWearingItem(itemId)) {
				return;
			}
		}
		if (player.hasPerk(Player.THE_ACCUMULATOR) && Misc.random(9) < 7) {
			return;
		}
		if (player.getItems().isWearingItem(12926)) {
			return;
		}
		if (player.getItems().isWearingItem(10499, player.playerCape) || player.getItems().isWearingItem(22109, player.playerCape) || player.getItems().isWearingItem(29024, player.playerCape) || player.getItems().isWearingItem(29023, player.playerCape) || player.getItems().isWearingItem(29022, player.playerCape) || player.getItems().isWearingItem(29021, player.playerCape) || SkillcapePerks.RANGING.isWearing(player)
	...
	public void deleteEquipment() {
		...
		if ( (Arrays.stream(WeaponDataConstants.THROWN).anyMatch(id -> id == thrownweapons) || player.usingOtherRangeWeapons) && !player.getItems().isWearingItem(Items.TOXIC_BLOWPIPE)) {
			if (player.getItems().isWearingItem(10499, player.playerCape) || player.getItems().isWearingItem(22109, player.playerCape) || player.getItems().isWearingItem(29024, player.playerCape) || player.getItems().isWearingItem(29023, player.playerCape) || player.getItems().isWearingItem(29022, player.playerCape) || player.getItems().isWearingItem(29021, player.playerCape)
