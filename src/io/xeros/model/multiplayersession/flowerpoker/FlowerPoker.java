...
public class FlowerPoker extends Multiplayer {
    ...
    public void request(Player requested) {
        ...
        if (!Boundary.isIn(player, Boundary.FLOWER_POKER_AREA)) {
            player.sendMessage("You can only gamble south of home");
            return;
        }
        ...
        if (player.getItems().getItemAmount(299) < 5 && !player.getItems().playerHasItem(300)) {
            player.sendMessage("<col=ff0000>You need 5 mithril seeds in order to gamble someone.");
            return;
        }
        if (requested.getItems().getItemAmount(299) < 5 && !requested.getItems().playerHasItem(300)) {
