...
public final class ObjectDef {
	...
	public static void loadConfig() throws IOException {
		...
		ByteStreamExt idx = new ByteStreamExt(getBuffer("loc.idx"));

		int highestObjectId = idx.readUnsignedWord();
		System.out.println("Total objects: " + highestObjectId);

		totalObjects = highestObjectId;
		streamIndices = new int[highestObjectId];

		int offset = 0;
		for (int i = 0; i < highestObjectId; i++) {
			final int size = idx.readUnsignedWord();
			if (size == 65535) break;
		...
		System.out.println(highestObjectId + " Object definitions loaded.");
	}
