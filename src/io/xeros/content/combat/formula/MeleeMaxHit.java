...
import io.xeros.model.entity.player.Food;
...
public class MeleeMaxHit {
	...
	public static int calculateBaseDamage(Player c) {
		...
		if (c.npcAttackingIndex > 0) {
			...
			if (npc != null) {
				if (c.getSlayer().hasSlayerHelmBoost(npc, CombatType.MELEE)) {
					...
					if (c.getItems().isWearingItem(12018, Player.playerAmulet) || c.zenithNeck()) {
		...
		if (c.fullVoidMeleeT2()) {
			base = (base * 1.12);
		}

		if (c.fullVoidMeleeT3()) {
			base = (base * 1.14);
		}

		if (c.fullVoidMeleeT4()) {
			base = (base * 1.17);
		}

		if (c.fullVoidMeleeT5()) {
			base = (base * 1.20);
		}
		...
		if (c.hasFollower) {
			if (c.petSummonId == 30015 && Misc.random(1) == 1) { //shadow warrior
				base *= 1.2;
			}
			if (c.petSummonId == 30115) { //dark shadow warrior
				base *= 1.25;
			}
		}
	...
	public static boolean hasObsidianEffect(Player c) {
		if (c.playerEquipment[2] != 11128 && !c.zenithNeck())
			return false;
	...
	public static int calculateMeleeAttack(Player c) {
		...
		if (c.npcAttackingIndex > 0 && c.trinketOfUndeadActive && Misc.linearSearch(Configuration.UNDEAD_NPCS, npc1.getNpcId()) != -1) { //salve amulet
			attackLevel += c.getLevelForXP(c.playerXP[Player.playerAttack]) * 0.15;
		}
		...
		if (c.npcAttackingIndex > 0 && c.playerEquipment[Player.playerAmulet] == 12018 && Misc.linearSearch(Configuration.UNDEAD_NPCS, npc1.getNpcId()) != -1 || c.npcAttackingIndex > 0 && c.zenithNeck() && Misc.linearSearch(Configuration.UNDEAD_NPCS, npc1.getNpcId()) != -1) { //salve amulet (ei)
			attackLevel += c.getLevelForXP(c.playerXP[Player.playerAttack]) * 0.20;
		}
		if (c.fullVoidMelee()) {
			attackLevel += c.getLevelForXP(c.playerXP[Player.playerAttack]) * 0.10;
		}
		if (ScytheOfVitur.usingScythe(c)) {
