...
public class RangeMaxHit extends RangeData {
	...
	public static int calculateRangeAttack(Player c) {
		...
		if (c.npcAttackingIndex > 0 && c.getItems().isWearingItem(Items.TWISTED_BOW) || c.npcAttackingIndex > 0 && c.getItems().isWearingItem(3694)) {
		...
		if (c.npcAttackingIndex > 0 && c.getSlayer().getTask().isPresent()) {
			...
			} else if (c.getItems().isWearingItem(12018, Player.playerAmulet) || c.zenithNeck()) {
	...
	public static int maxHit(Player c) {
		int rangedStrength;
		if (wearingCrystalBow(c) || c.getItems().isWearingItem(Items.CRAWS_BOW, Player.playerWeapon) || c.getItems().isWearingItem(Items.BOW_OF_FAERDHINEN, Player.playerWeapon) || c.getItems().isWearingItem(Items.BOW_OF_FAERDHINEN_T2, Player.playerWeapon) || c.getItems().isWearingItem(Items.BOW_OF_FAERDHINEN_T3, Player.playerWeapon) || c.getItems().isWearingItem(Items.BOW_OF_FAERDHINEN_T4, Player.playerWeapon) || c.getItems().isWearingItem(Items.BOW_OF_FAERDHINEN_T5, Player.playerWeapon)) {
		...
		if (c.fullVoidRangeT2()) {
			b += 0.12;
		}

		if (c.fullVoidRangeT3()) {
			b += 0.14;
		}

		if (c.fullVoidRangeT4()) {
			b += 0.17;
		}

		if (c.fullVoidRangeT5()) {
			b += 0.20;
		}

		if (c.hasPerk(Player.CORRUPT_RANGER) && c.wearingCrystalBody() && c.wearingCrystalHelm() && c.wearingCrystalLegs()) {
			b += 0.05;
		}
		...
		if (c.hasFollower) {
			if (c.petSummonId == 30016 && Misc.random(1) == 1) { //shadow archer
				b *= 1.20;
			}
			if (c.petSummonId == 30116) { //dark shadow archer
				b *= 1.25;
			}
		}
		...
		if (c.npcAttackingIndex > 0) {
			...
			if (c.getItems().isWearingItem(Items.TWISTED_BOW) || c.getItems().isWearingItem(Items.CORRUPT_TWISTED_BOW) || c.getItems().isWearingItem(3714) || c.getItems().isWearingItem(3715) || c.getItems().isWearingItem(3716)) {
