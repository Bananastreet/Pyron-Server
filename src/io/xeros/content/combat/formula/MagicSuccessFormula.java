...
public class MagicSuccessFormula {
    ...
    public static int getHitChance(Player attacker, Entity defender) {
        ...
        if (attacker.npcAttackingIndex > 0 && attacker.getSlayer().getTask().isPresent()) {
            ...
            if (npc != null) {
                // Salve amulet
                if (attacker.getItems().isWearingItem(12018, Player.playerAmulet) && Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1 || attacker.zenithNeck() && Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1) {
