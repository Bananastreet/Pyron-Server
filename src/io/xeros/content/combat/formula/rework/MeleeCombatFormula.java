...
public class MeleeCombatFormula implements CombatFormula {
    ...
    private double getNpcMultipliers(Player attacker, NPC defender) {
        ...
        if (attacker.getSlayer().isTaskNpc(defender)) {
            ...
            if (attacker.getItems().isWearingItem(29018)) {
                multiplier += 0.05;
            }

            if (attacker.getItems().isWearingItem(29019)) {
                multiplier += 0.10;
            }
    ...
    private double getEffectiveStrengthLevel(Player player) {
        ...
        if (player.torvaHelm(player) && player.torvaBody(player) && player.torvaLegs(player) && player.hasPerk(Player.TORVA_WARRIOR)) {
            effectiveLevel *= 1.15;
        }
        ...
        if (player.fullVoidMeleeT2()) {
            effectiveLevel *= 1.12;
        }

        if (player.fullVoidMeleeT3()) {
            effectiveLevel *= 1.14;
        }

        if (player.fullVoidMeleeT4()) {
            effectiveLevel *= 1.17;
        }

        if (player.fullVoidMeleeT5()) {
            effectiveLevel *= 1.20;
        }
    ...
    private int getEffectiveAttackLevel(Player attacker) {
        ...
        if (attacker.fullVoidMeleeT2()) {
            effectiveLevel *= 1.12;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (attacker.fullVoidMeleeT3()) {
            effectiveLevel *= 1.14;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (attacker.fullVoidMeleeT4()) {
            effectiveLevel *= 1.17;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (attacker.fullVoidMeleeT5()) {
            effectiveLevel *= 1.20;
            effectiveLevel = Math.floor(effectiveLevel);
        }
