...
public class MagicCombatFormula implements CombatFormula {
    ...
    private double getEffectiveAttackLevel(Player player) {
        ...
        if (player.fullVoidMagicT2()) {
            effectiveLevel *= 1.50;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (player.fullVoidMagicT3()) {
            effectiveLevel *= 1.55;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (player.fullVoidMagicT4()) {
            effectiveLevel *= 1.60;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (player.fullVoidMagicT5()) {
            effectiveLevel *= 1.65;
            effectiveLevel = Math.floor(effectiveLevel);
        }
    ...
    public int getMaxHit(Entity attacker, Entity defender, double specialAttackMultiplier,
        ...
        if (attacker.isPlayer()) {
            ...
            if (attacker.getPermAttributes().getOrDefault(Player.ARCANE_MAGE)) {
                hit += 5;
            }

            if (player.ancestralHat() && player.ancestralTop() && player.ancestralBottom() && player.hasPerk(Player.ANCESTRAL_MAGE)) {
                hit *= 1.15;
            }
    ...
    public boolean hasSanguinestiCharges(Player player) {
        if (player.getItems().hasAnyItem(Player.playerWeapon, Items.SANGUINESTI_STAFF,
                Items.SANGUINESTI_STAFF_UNCHARGED) || player.getItems().hasAnyItem(Player.playerWeapon, Items.SANGUINESTI_STAFF_T2, 29043, 29044, 29045,
