...
public class RangeCombatFormula implements CombatFormula {
    ...
    private double getEffectiveRangeAttackLevel(Player player) {
        ...
        if (player.fullVoidRangeT2()) {
            effectiveLevel *= 1.12;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (player.fullVoidRangeT3()) {
            effectiveLevel *= 1.14;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (player.fullVoidRangeT4()) {
            effectiveLevel *= 1.17;
            effectiveLevel = Math.floor(effectiveLevel);
        }

        if (player.fullVoidRangeT5()) {
            effectiveLevel *= 1.20;
            effectiveLevel = Math.floor(effectiveLevel);
        }
    ...
    private double applyAttackSpecials(Player attacker, Entity defender, double base,
        ...
        if (specialAttackMultiplier == 1.0) {
            ...
            // Twisted Bow Effect
            if (attacker.getItems().isWearingItem(Items.TWISTED_BOW, Player.playerWeapon) || attacker.getItems().isWearingItem(Items.CORRUPT_TWISTED_BOW, Player.playerWeapon) || attacker.getItems().isWearingItem(3714, Player.playerWeapon) || attacker.getItems().isWearingItem(3715, Player.playerWeapon) || attacker.getItems().isWearingItem(3716, Player.playerWeapon)) {
                if (defender.isNPC()) {
    ...
    private double getEffectiveRangeLevel(Player player, Entity target) {
        ...
        if (player.fullEliteVoidRange()) {
            ...
            effectiveLevel *= 1.1;
            effectiveLevel = Math.floor(effectiveLevel);
        } else if (player.fullVoidRangeT2()) {
            effectiveLevel *= 1.12;
            effectiveLevel = Math.floor(effectiveLevel);
        } else if (player.fullVoidRangeT3()) {
            effectiveLevel *= 1.14;
            effectiveLevel = Math.floor(effectiveLevel);
        } else if (player.fullVoidRangeT5()) {
            effectiveLevel *= 1.17;
            effectiveLevel = Math.floor(effectiveLevel);
        } else if (player.fullVoidRangeT5()) {
            effectiveLevel *= 1.20;
    ...
    private int applyRangedSpecials(Entity attacker, Entity defender, int base, double specialAttackMultiplier,
        ...
        if (player != null && specialAttackMultiplier == 1.0) {
            ...
            if (player.getItems().isWearingItem(Items.TWISTED_BOW, Player.playerWeapon) || player.getItems().isWearingItem(Items.CORRUPT_TWISTED_BOW, Player.playerWeapon) || player.getItems().isWearingItem(3714, Player.playerWeapon) || player.getItems().isWearingItem(3715, Player.playerWeapon) || player.getItems().isWearingItem(3716, Player.playerWeapon)) {
                if (defender.isNPC()) {
                    int magicLevel = 1;
                    if (defender.isPlayer()) {
    ...
    private double getNpcMultipliers(Player attacker, NPC defender) {
        ...
        multiplier += getUndeadTrinketMultiplier(attacker, defender);
        ...
        if (attacker.hasFollower && attacker.petSummonId == 12921 && Boundary.isIn(attacker, Boundary.ZULRAH)) {
            multiplier += 0.15;
        }

        if (attacker.petSummonId == 13177) {
