...
public class MagicMaxHit {
	...
	public static int mageAttack(Player c) {
		...
		if (c.npcAttackingIndex > 0 && c.getSlayer().getTask().isPresent()) {
			...
			if (npc != null) {
				if (c.trinketOfUndeadActive) {
					modifier += .15;
				}
				// Salve amulet
				if (c.getItems().isWearingItem(12018, Player.playerAmulet) && Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1 || c.zenithNeck() && Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1) {
					modifier += .20;
				// Slayer helmet
	...
	public static int magiMaxHit(Player c) {
		...
		if (c.hasFollower) {
			if (c.petSummonId == 30017 && Misc.random(1) == 1) { //shadow wizard
				damageMultiplier += .20;
			}
			if (c.petSummonId == 30117) { //dark shadow wizard
				damageMultiplier += .25;
			}
		}
		if (c.oldSpellId > -1) {
