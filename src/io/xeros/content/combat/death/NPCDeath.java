...
public class NPCDeath {
    ...
    public static void dropItemsFor(NPC npc, Player c, int npcId) {
        ...
        if (npcId == 319) {
            for (int j = 0; j < PlayerHandler.players.length; j++) {
                if (PlayerHandler.players[j] != null) {
                    ...
                    if (player.corpDamage > 100) {
                        Location3D playerLocation = new Location3D(player.getX(), player.getY(), player.getHeight());
                        Server.getDropManager().create(player, npc, playerLocation, 1, 319);
                        player.getNpcDeathTracker().add(NpcDef.forId(npcId).getName(), NpcDef.forId(npcId).getCombatLevel(), 5);
                        player.sendMessage("You dealt "+ player.corpDamage + " damage that kill.");
                        player.corpDamage = 0;
                    }
                }
            }
        ...
        if (npcId == VoteBoss.bossID) {
            VoteBoss.handleReward();
            return;
        }

        if (npcId == Abomination.bossID) {
            Abomination.handleReward();
            return;
        }
        ...
        if (npcId == AvatarOfDestruction.bossID) {
            AvatarOfDestruction.handleReward(location, npc, amountOfDrops, npcId);
            return;
        }

        if (npcId == inadequacy.bossID) {
            System.out.println("boop");
            inadequacy.handleReward(location, npc, amountOfDrops, npcId);
            return;
        }
        ...
        if (c.doubleDropScrollActive && Misc.hasOneOutOf(5)) {
            c.sendMessage("@red@Your double drop scroll gives you an additional drop");
            Server.getDropManager().create(c, npc, location, amountOfDrops, npcId);
        }
    }
    ...
    public static void announceKc(Player player, GameItem item, int kc) {
        PlayerHandler.dropMessage(player.getDisplayNameFormatted() + " received a drop: " +
                "" + item.getAmount() + " x " + ItemDef.forId(item.getId()).getName() + " at <col=E9362B>" + kc  + "</col><col=9E4A00> kills.</col>");
        DiscordWebhook webhook = new DiscordWebhook(Configuration.DropsWebhook);
        webhook.addEmbed(new DiscordWebhook.EmbedObject()
                .setTitle(player.getDisplayName())
                .setDescription("" + player.getDisplayName() + " has received " + item.getAmount() + "x " + ItemDef.forId(item.getId()).getName() + " with "+ kc +" killcount.")
                .setColor(Color.RED));
        try {
