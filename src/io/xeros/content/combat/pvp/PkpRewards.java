...
//                    && killer.getBH().hasTarget() && killer.getBH().getTarget().getName().equalsIgnoreCase(dying.playerName)) {
        ...
        if (Boundary.isIn(dying, Boundary.WILDERNESS_PARAMETERS)) {
            ...
            if (canReceiveRewards) {
                ...
                int random = 30;
                killer.pkp += random;
                giveBonusPkp(killer, DonatorRank.determineBonusPkp(DonatorRank.getHighestRank(killer)));
            }
    ...
    private static void giveBonusPkp(Player player, int bonus) {
        player.pkp += bonus;
        player.sendMessage("[@red@PKP@bla@] You are rewarded an extra@red@ " + bonus + " pkp @bla@for your donator rank.");
    }
