...
public class AttackNpcCheck {
    ...
    public static boolean check(Player c, Entity targetEntity, boolean sendMessages) {
        ...
        if (Arrays.stream(RangeData.NO_ARROW_DROP).anyMatch(id -> id == weapon) && c.playerEquipment[13] > 0) {
            if (Arrays.stream(Configuration.blessings).anyMatch(id -> id == c.playerEquipment[13]) || Arrays.stream(RangeData.karilsbows).anyMatch(id -> id == weapon)) {

            } else {
                c.sendMessage("You can not use this weapon with arrows or bolts equipped.");
                return false;
            }
        }
...
//        }
        ...
        if (c.getItems().isWearingItem(10858, Player.playerWeapon)) {
            if (Misc.linearSearch(Configuration.UNDEAD_NPCS, npc.getNpcId()) != -1) {

            } else {
                c.sendMessage("You can only use a shadow sword against undead npcs.");
                return false;
            }
        }

       /* if (npc.getNpcId() == VoteBoss.bossID && !VoteBoss.voteBossActive || npc.getNpcId() == VoteBoss.bossID && !VoteBoss.voteBossEnabled) {
            c.sendMessage("The vote boss can not be attacked right now. "+ VoteBoss.votesRemaining()+" votes required.");
            return false;
        }*/

        if (npc.getNpcId() == Abomination.bossID && !Abomination.abominationActive || npc.getNpcId() == Abomination.bossID && !Abomination.abominationEnabled) {
            c.sendMessage("The abomination can not be attacked right now. $"+ Abomination.amountRemaining()+" donations required.");
            return false;
        }

        if (npc.getNpcId() == VoteBoss.bossID ||  npc.getNpcId() == Abomination.bossID) {
            return true;
        }

        if (npc.getNpcId() == 2549 || npc.getNpcId() == 2550 || npc.getNpcId() == 2551 || npc.getNpcId() == 2552 || npc.getNpcId() == 2553 || npc.getNpcId() == 2554) {
            if (c.getItems().isWearingItem(7806) || c.getItems().isWearingItem(7807) || c.getItems().isWearingItem(7808) || c.getItems().isWearingItem(7809) || c.getItems().isWearingItem(7810)) {

            } else {
                c.sendMessage("You can only attack seasonal monsters with a seasonal weapon.");
                return false;
            }
        }
        ...
        if (c.getItems().isWearingItem(7806) || c.getItems().isWearingItem(7807) || c.getItems().isWearingItem(7808) || c.getItems().isWearingItem(7809) || c.getItems().isWearingItem(7810)) {
            if (npc.getNpcId() != 2549 && npc.getNpcId() != 2550 && npc.getNpcId() != 2551 && npc.getNpcId() != 2552 && npc.getNpcId() != 2553 && npc.getNpcId() != 2554) {
                c.sendMessage("You can only attack seasonal npcs with this weapon.");
                return false;
            }
        }

        if (Boundary.isIn(npc, Boundary.SEASONAL_CAVE) && !Configuration.seasonalMass) {
            c.sendMessage("The seasonal mass event is not currently active.");
            c.getPA().spellTeleport(3095, 3507, 0, false);
            return false;
        }
        ...
        if (!npc.getPosition().inMulti()) {
            ...
            if (!c.getPosition().inMulti() && (c.underAttackByPlayer > 0 && c.underAttackByNpc != npc.getIndex())
               ...
               if (c.getDisplayName().equalsIgnoreCase("bubly")) {
                   c.sendMessage("Underattackbynpc npcid: "+ npc.getNpcId());
                   c.sendMessage("Underattackbynpc index: "+ npc.getIndex());
                   c.sendMessage("Underattackbynpc: "+ c.underAttackByNpc);
                   c.sendMessage("Underattackbyplayer: "+ c.underAttackByPlayer);
               }
               if (c.underAttackByNpc != 2412 && c.underAttackByNpc != 2412 && c.underAttackByNpc != 2410 && c.underAttackByNpc != 2428) {
        ...
        if (npc.getNpcId() == 4922 || npc.getNpcId() == 5129 || npc.getNpcId() == 7860) {
        ...
        if (npc.getNpcId() == 499) {
            if (!c.getSlayer().onTask("thermonuclear smoke devil") && !c.getSlayer().onTask("smoke devil") && c.bc1 != 499 && c.bc2 != 499 && c.bc3 != 499 && c.bc4 != 499 && c.bc5 != 499) {
        ...
        if (npc.getNpcId() == 9040) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted rat"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }

        if (npc.getNpcId() == 9041) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted spider"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }

        if (npc.getNpcId() == 9042) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted bat"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }

        if (npc.getNpcId() == 9043) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted unicorn"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }
        if (npc.getNpcId() == 9044) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted scorpion"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }
        if (npc.getNpcId() == 9045) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted wolf"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }
        if (npc.getNpcId() == 9046) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted bear"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }
        if (npc.getNpcId() == 9047) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted dragon"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }

        if (npc.getNpcId() == 9048) {//rat
            if (!(c.getSlayer().getTask().isPresent()) || (c.getSlayer().getTask().isPresent() && !c.getSlayer().getTask().get().getPrimaryName().equals("corrupted dark beast"))) {
                sendCheckMessage(c, sendMessages, "The creature does not seem interested.");
                return false;
            }
        }
