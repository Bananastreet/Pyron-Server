...
public class AttackPlayerCheck {
    ...
    public static boolean check(Player c, Entity targetEntity, boolean sendMessages) {
        ...
        String name = ItemAssistant.getItemName(c.getItems().getWeapon());

        if (Boundary.isIn(c, Boundary.WILDERNESS)) {
            if (name.toLowerCase().contains("t2") || name.toLowerCase().contains("t3") || name.toLowerCase().contains("t4") || name.toLowerCase().contains("t5")) {
                c.sendMessage("You can not attack someone with a T2, T3, T4, or T5 weapon.");
                return false;
            }

            if (c.getItems().isWearingItem(10858)) {
                c.sendMessage("You can not attack another player with the shadow sword.");
                return false;
            }

            if (c.getItems().isWearingItem(11061)) {
                c.sendMessage("You can not attack another player with the ancient mace");
                return false;
            }

            if (c.getItems().isWearingItem(25872) || c.getItems().isWearingItem(24551) || c.getItems().isWearingItem(25867)) {
                c.sendMessage("You can not attack another player with this weapon.");
                return false;
            }
        }
