...
import io.xeros.content.bosses.inadequacy;
...
public abstract class HitDispatcher {
    ...
    public static double crystalReturnDamage(Player attacker) {
        double multiplier = 1.0;

        if (attacker.wearingFaerdhinenBow()) {

            if (attacker.wearingCrystalHelm()) {
                multiplier += 0.025;
            }

            if (attacker.wearingCrystalBody()) {
                multiplier += 0.075;
            }

            if (attacker.wearingCrystalLegs()) {
                multiplier += 0.05;
            }

            if (attacker.hasPerk(Player.CRYSTAL_RANGER) && attacker.wearingCrystalBody() && attacker.wearingCrystalHelm() && attacker.wearingCrystalLegs()) {
                multiplier += 0.10;
            }
    ...
    public static double crystalReturnAccuracy(Player attacker) {
        double boost = 1;
        if (attacker.getItems().getWeapon() == Items.BOW_OF_FAERDHINEN || attacker.getItems().getWeapon() == Items.BOW_OF_FAERDHINEN_T2 || attacker.getItems().getWeapon() == Items.BOW_OF_FAERDHINEN_T3 || attacker.getItems().getWeapon() == Items.BOW_OF_FAERDHINEN_T4 || attacker.getItems().getWeapon() == Items.BOW_OF_FAERDHINEN_T5) {
    ...
    private void playerHitEntity(CombatType combatType, Special special, boolean applyingMultiHitAttack) {
        ...
        if (combatType.equals(CombatType.MELEE)) {
            ...
            if (attacker.getItems().playerHasItem(25990, 1) || attacker.getItems().playerHasItem(26146, 1)) {
                if (Misc.random(4) == 3) {
                    damage += 5;
                }
            }

            var wieldingAbyssalWeapon = Arrays.stream(WeaponDataConstants.WHIPS).anyMatch(i -> attacker.getItems().isWearingItem(i));
            if (wieldingAbyssalWeapon && attacker.hasPerk(Player.ABYSSAL_WARRIOR) && damage > 0) {
                damage += 2;
            }
            ...
            damage *= crystalReturnDamage(attacker);
            maximumAccuracy *= crystalReturnAccuracy(attacker);
            ...
            if (attacker.getItems().playerHasItem(26065, 1) || attacker.getItems().playerHasItem(26146, 1)) {
                if (Misc.random(4) == 3) {
                    //attacker.sendMessage("Your damage before was "+damage);
                    damage += 3;
                    // attacker.sendMessage("Your damage after was "+damage);
                }
            }
            ...
            if (attacker.getItems().playerHasItem(26128, 1) || attacker.getItems().playerHasItem(26146, 1)) {
               if (Misc.random(4) == 3) {
                    //attacker.sendMessage("Your damage before was "+damage);
                    damage += 6;
                    //attacker.sendMessage("Your damage after was "+damage);
        ...
        if (defender != null && defender.isNPC()) {
            ...
            if (n.getNpcId() == 319) {
                attacker.corpDamage += damage;
            }

            if (n.getNpcId() == 7414) {
                attacker.voteBossDamage += damage;
                if (attacker.getMode().getType() == ModeType.ROGUE_HARDCORE_IRONMAN || attacker.getMode().getType() == ModeType.ROGUE || attacker.getMode().getType() == ModeType.ROGUE_IRONMAN) {
                    attacker.voteBossDamage += (damage * 0.5);
                }
            }

            if (n.getNpcId() == 8262) {
                attacker.abominationDamage += damage;
                if (attacker.getMode().getType() == ModeType.ROGUE_HARDCORE_IRONMAN || attacker.getMode().getType() == ModeType.ROGUE || attacker.getMode().getType() == ModeType.ROGUE_IRONMAN) {
                    attacker.abominationDamage += (damage * 0.5);
                }
            }

            if (n.getNpcId() == 10532) {
                attacker.avatarDamage += damage;
            }

            if (n.getNpcId() == inadequacy.bossID) {
                attacker.inadequacyDamage += damage;
            }

            if (n.getNpcId() == 7310) {
                attacker.brassicanDamage += damage;
            }
        ...
        if (damage > 0 && attacker.getItems().isWearingItem(24780) && Misc.hasOneOutOf(5) || damage > 0 && attacker.zenithNeck() && Misc.hasOneOutOf(5)) {
                int healAmount = damage / 4;
                if (attacker.getItems().playerHasItem(26125, 1) || attacker.getItems().playerHasItem(26146, 1)) {
                    healAmount *= 3;
                }
                attacker.getHealth().increase(healAmount);
        }
        if (damage > 0 && attacker.getItems().isWearingItem(10559) && Misc.hasOneOutOf(5)) {
            int healAmount = damage / 6;
            attacker.getHealth().increase(healAmount);
        }
    }
    ...
    private void dropArrows() {
        ...
        if (weaponId == Items.BOW_OF_FAERDHINEN) {
            return;
        }
        if (weaponId == Items.BOW_OF_FAERDHINEN_T2) {
            return;
        }
        if (weaponId == Items.BOW_OF_FAERDHINEN_T3) {
            return;
        }
        if (weaponId == Items.BOW_OF_FAERDHINEN_T4) {
            return;
        }
        if (weaponId == Items.BOW_OF_FAERDHINEN_T5) {
            return;
        }
    ...
    private void dropArrow(int arrowId) { // TODO delete arrows from player
        ...
        if (attacker.playerEquipment[Player.playerCape] == 10499 || attacker.getItems().isWearingItem(22109,
                ...
                || attacker.getItems().isWearingItem(29024, Player.playerCape)
                || attacker.getItems().isWearingItem(29023, Player.playerCape)
                || attacker.getItems().isWearingItem(29022, Player.playerCape)
                || attacker.getItems().isWearingItem(29021, Player.playerCape)
