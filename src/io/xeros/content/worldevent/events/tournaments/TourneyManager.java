package io.xeros.content.worldevent.events.tournaments;
...
public class TourneyManager {
    ...
    public void leaveLobby(Player player, boolean xlog) {
        if (!player.spectatingTournament && isParticipant(player)) {
            ...
            if (xlog)
                player.getPA().forceMove(3112, 3508, 0, true);
            else
                player.getPA().movePlayer(new Coordinate(3112, 3508));
    ...
    public void handleDeath(String playerName, boolean logout) {
        ...
        if (player != null) {
            ...
            player.getPA().forceMove(3112, 3508, 0, logout);
        }
        ...
        for (int i = victors.size() - 1; i >= 0; i--) {
            ...
            if (player != null) {
                ...
                if (i == 2) {
                    ...
                    if (player.hasPerk(Player.TOURNY_BOOSTER)) {
                        player.tournamentPoints += 2;
                        player.sendMessage("You receive 2 extra points from your tourny booster perk.");
                    }
                    ...
                    if (Configuration.leaderboardEnabled) {
                        LeaderboardSerialisation.INSTANCE.updateCount(player, LeaderboardData.MISCELLANEOUS,
                                player.tournamentWins, io.xeros.content.leaderboard.impl.Misc.TOURNAMENT_WINS);
                    }
                    player.sendMessage("@blu@You have gained an extra point for participating.");
                    player.attacking.reset();
