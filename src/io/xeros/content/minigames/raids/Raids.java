...
//
    ...
    public int upgradeTokenChance(Player player, int damage) {
        int chance = 40;
    ...
    public int rareKeyChance(Player player, int damage) {
        int chance = 40;
        if (damage > 4000) {
            chance -= 10;
        } else if (damage > 3000) {
        ...
        if (player.petSummonId == 20851) {
            chance -= 3;
        }
        if (Wogw._20_PERCENT_DROP_RATE_TIMER > 0) {
            chance -= 2;
        }
        if (player.hasPerk(Player.COX_BOOSTER)) {
            chance -= 2;
        }
        ...
        //chance *= (DropManager.getModifierRaids(player));
    ...
    public void giveReward(Player player, Boolean kronosReward) {
        ...
        player.sendMessage("Your odds of receiving a rare key were 1 in "+ rareKeyChance(player, getDamage(player)));
        ...
        ChallengeHandler.killChecker(player, 8184, false);
        ...
        if (Misc.hasOneOutOf(upgradeTokenChance(player, getDamage(player)))) {
            player.getItems().addItemUnderAnyCircumstance(23497, 1);
            player.sendMessage("@red@You have received an upgrade token.");
            PlayerHandler.executeGlobalMessage("@bla@[@blu@RAIDS@bla@] " + player.getDisplayName() + "@pur@ has just received a @bla@Upgrade Token!");
        }

        int keyAmount = 1;
        if (kronosReward) keyAmount = 2;

        if (getDamage(player) > 20) {
            int randomTickets = Misc.random(250, 500);
            player.getItems().addItemUnderAnyCircumstance(SeasonalTickets.ticketID, randomTickets);
            player.sendMessage("You've been given "+ randomTickets +"x seasonal tickets for completing the raid.");
        }

        if (Misc.hasOneOutOf(rareKeyChance(player, getDamage(player)))) {
            ...
            PlayerHandler.executeGlobalMessage("@bla@[@blu@RAIDS@bla@] " + player.getDisplayName() + "@pur@ has just received "+ keyAmount +"x @bla@Rare Raids Key!");
        } else {
		...
		case 29778:
			...
			if (Boundary.isIn(player, Boundary.FULL_RAIDS)) {
				...
				if (Hespori.activeKronosSeed == true) {
                   ...
                   player.sendMessage("@red@The @gre@server boost@red@ doubles your chances!" );
                }
			}
            resetDamage(player);
