...
public class UseItem {
	...
	public static void ItemonObject(Player c, int objectID, int objectX, int objectY, int itemId) {
		...
		if (def != null) {
			...
			if (def.name != null && def.name.toLowerCase().contains("bank") || objectID == 25808) {
					//ItemDefinition definition = ItemDefinition.forId(itemId);
					boolean stackable = ItemDef.forId(itemId).isStackable();
					if (stackable) {
						unNoteItems(c, itemId, c.getItems().getItemAmount(itemId));
					} else {
		...
		switch (objectID) {
			case 42966: //ancient forge nex
				switch (itemId) {
					case Items.BANDOS_CHESTPLATE:
						if (c.getItems().freeSlots() < 3) {
							c.sendMessage("You need at least 3 free slots to do this.");
							return;
						}
						c.start(new DialogueBuilder(c).option("Break down the Bandos chestplate?",
								new DialogueOption("Yes.", player -> {
									if (!player.getItems().playerHasItem(11832)) { return; }
									player.getItems().deleteItem(11832, 1);
									player.getItems().addItem(26394, 3);
					...
					case Items.BANDOS_TASSETS:
						...
						c.start(new DialogueBuilder(c).option("Break down the Bandos tassets?",
								new DialogueOption("Yes.", player -> {
									if (!player.getItems().playerHasItem(11834)) { return; }
									player.getItems().deleteItem(11834, 1);
									player.getItems().addItem(26394, 2);
									player.getPA().closeAllWindows();
						...
						break;
					case 26376: //torva helm damaged
						if (c.getItems().playerHasItem(26376) && c.getItems().playerHasItem(26394)) {
							c.getItems().deleteItem(26376, 1);
							c.getItems().deleteItem2(26394, 1);
							c.getItems().addItem(26382, 1);
						} else {
							c.sendMessage("You need Torva full helm (damaged) and 1 Bandosian component to do this.");
							return;
						}
						break;
					case 26378: //torva platebody damaged
						if (c.getItems().playerHasItem(26378) && c.getItems().playerHasItem(26394, 2)) {
							c.getItems().deleteItem(26378, 1);
							c.getItems().deleteItem2(26394, 2);
							c.getItems().addItem(26384, 1);
						} else {
							c.sendMessage("You need Torva platebody (damaged) and 2 Bandosian components to do this.");
							return;
						}
						break;
					case 26380: //torva platelegs damaged
						if (c.getItems().playerHasItem(26380) && c.getItems().playerHasItem(26394, 2)) {
							c.getItems().deleteItem(26380, 1);
							c.getItems().deleteItem2(26394, 2);
							c.getItems().addItem(26386, 1);
						} else {
							c.sendMessage("You need Torva platelegs (damaged) and 2 Bandosian components to do this.");
							return;
						}
						break;
				}
		...
		case 5249:
			...
			c.getBonfire().start(log, c.getItems().getItemAmount(itemId), objectX, objectY);
			break;
	...
	public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
		...
		if (itemUsed == 29074) { // scythe of vitur

			if (useWith == 22325 && c.getItems().playerHasItem(22325) && c.getItems().playerHasItem(29074)) { //t2 scythe
				c.getItems().deleteItem(22325, 1);
				c.getItems().deleteItem(29074, 1); //kit
				c.getItems().addItem(29070, 1);
			}

			if (useWith == 29070 && c.getItems().playerHasItem(29070) && c.getItems().playerHasItem(29074)) { //t3 scythe
				c.getItems().deleteItem(29070, 1);
				c.getItems().deleteItem(29074, 1); //kit
				c.getItems().addItem(29071, 1);
			}

			if (useWith == 29071 && c.getItems().playerHasItem(29071) && c.getItems().playerHasItem(29074)) { //t4 scythe
				c.getItems().deleteItem(29071, 1);
				c.getItems().deleteItem(29074, 1); //kit
				c.getItems().addItem(29072, 1);
			}

			if (useWith == 29072 && c.getItems().playerHasItem(29072) && c.getItems().playerHasItem(29074)) { //t5 scythe
				c.getItems().deleteItem(29072, 1);
				c.getItems().deleteItem(29074, 1); //kit
				c.getItems().addItem(29073, 1);
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Scythe of Vitur to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Scythe of Vitur to T5.")
						.setColor(Color.RED));
				try {
		...
		if (itemUsed == 29069) { //torva

			if (useWith == 26382 && c.getItems().playerHasItem(26382) && c.getItems().playerHasItem(29069)) { //t2 helm
				c.getItems().deleteItem(26382, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29057, 1);
			}

			if (useWith == 29057 && c.getItems().playerHasItem(29057) && c.getItems().playerHasItem(29069)) { //t3 helm
				c.getItems().deleteItem(29057, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29060, 1);
			}

			if (useWith == 29060 && c.getItems().playerHasItem(29060) && c.getItems().playerHasItem(29069)) { //t4 helm
				c.getItems().deleteItem(29060, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29063, 1);
			}

			if (useWith == 29063 && c.getItems().playerHasItem(29063) && c.getItems().playerHasItem(29069)) { //t5 helm
				c.getItems().deleteItem(29063, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29066, 1);
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Torva Helm to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Torva Helm to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
			...
			if (useWith == 26384 && c.getItems().playerHasItem(26384) && c.getItems().playerHasItem(29069)) { //t2 plate
				c.getItems().deleteItem(26384, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29058, 1);
			}

			if (useWith == 29058 && c.getItems().playerHasItem(29058) && c.getItems().playerHasItem(29069)) { //t3 plate
				c.getItems().deleteItem(29058, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29061, 1);
			}

			if (useWith == 29061 && c.getItems().playerHasItem(29061) && c.getItems().playerHasItem(29069)) { //t4 plate
				c.getItems().deleteItem(29061, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29064, 1);
			}

			if (useWith == 29064 && c.getItems().playerHasItem(29064) && c.getItems().playerHasItem(29069)) { //t5 plate
				c.getItems().deleteItem(29064, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29067, 1);
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Torva Platebody to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Torva Platebody to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
			...
			if (useWith == 26386 && c.getItems().playerHasItem(26386) && c.getItems().playerHasItem(29069)) { //t2 legs
				c.getItems().deleteItem(26386, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29059, 1);
			}

			if (useWith == 29059 && c.getItems().playerHasItem(29059) && c.getItems().playerHasItem(29069)) { //t3 legs
				c.getItems().deleteItem(29059, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29062, 1);
			}

			if (useWith == 29062 && c.getItems().playerHasItem(29062) && c.getItems().playerHasItem(29069)) { //t4 legs
				c.getItems().deleteItem(29062, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29065, 1);
			}

			if (useWith == 29065 && c.getItems().playerHasItem(29065) && c.getItems().playerHasItem(29069)) { //t5 legs
				c.getItems().deleteItem(29065, 1);
				c.getItems().deleteItem(29069, 1); //scroll
				c.getItems().addItem(29068, 1);
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Torva Platelegs to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Torva Platelegs to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 29012) {
			...
			if (useWith == 22324 && c.getItems().playerHasItem(22324) && c.getItems().playerHasItem(29012)) { //t2 rapier
				c.getItems().deleteItem(29012, 1);
				c.getItems().deleteItem(22324, 1);
				c.getItems().addItem(3692, 1);
				c.sendMessage("You upgrade your ghrazi rapier to tier 2.");
			}

			if (useWith == 3692 && c.getItems().playerHasItem(3692) && c.getItems().playerHasItem(29012)) { //t3 rapier
				c.getItems().deleteItem(29012, 1);
				c.getItems().deleteItem(3692, 1);
				c.getItems().addItem(29013, 1);
				c.sendMessage("You upgrade your ghrazi rapier to tier 3.");
			}

			if (useWith == 29013 && c.getItems().playerHasItem(29013) && c.getItems().playerHasItem(29012)) { //t3 rapier
				c.getItems().deleteItem(29012, 1);
				c.getItems().deleteItem(29013, 1);
				c.getItems().addItem(29014, 1);
				c.sendMessage("You upgrade your ghrazi rapier to tier 4.");
			}

			if (useWith == 29014 && c.getItems().playerHasItem(29014) && c.getItems().playerHasItem(29012)) { //t3 rapier
				c.getItems().deleteItem(29012, 1);
				c.getItems().deleteItem(29014, 1);
				c.getItems().addItem(29015, 1);
				c.sendMessage("You upgrade your ghrazi rapier to tier 5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Ghrazi Rapier to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Ghrazi Rapier to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 25481) { //infernal cape upgrade kit

			if (useWith == 21295 && c.getItems().playerHasItem(21295) && c.getItems().playerHasItem(25481)) { //t2 infernal
				c.getItems().deleteItem(21295, 1);
				c.getItems().deleteItem(25481, 1);
				c.getItems().addItem(29016, 1);
				c.sendMessage("You upgrade your infernal cape to tier 2.");
			}

			if (useWith == 29016 && c.getItems().playerHasItem(29016) && c.getItems().playerHasItem(25481)) { //t3 infernal
				c.getItems().deleteItem(29016, 1);
				c.getItems().deleteItem(25481, 1);
				c.getItems().addItem(29017, 1);
				c.sendMessage("You upgrade your infernal cape to tier 3.");
			}

			if (useWith == 29017 && c.getItems().playerHasItem(29017) && c.getItems().playerHasItem(25481)) { //t4 infernal
				c.getItems().deleteItem(29017, 1);
				c.getItems().deleteItem(25481, 1);
				c.getItems().addItem(29018, 1);
				c.sendMessage("You upgrade your infernal cape to tier 4.");
			}

			if (useWith == 29018 && c.getItems().playerHasItem(29018) && c.getItems().playerHasItem(25481)) { //t4 infernal
				c.getItems().deleteItem(29018, 1);
				c.getItems().deleteItem(25481, 1);
				c.getItems().addItem(29019, 1);
				c.sendMessage("You upgrade your infernal cape to tier 5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Infernal Cape to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Infernal Cape to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 3461) { //ancestral upgrade kit

			if (useWith == 21018 && c.getItems().playerHasItem(21018) && c.getItems().playerHasItem(3461)) { //t2 hat creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(21018, 1);
				c.getItems().addItem(29000, 1);
				c.sendMessage("You upgrade your ancestral hat to tier 2.");
			}

			if (useWith == 21021 && c.getItems().playerHasItem(21021) && c.getItems().playerHasItem(3461)) { //t2 body creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(21021, 1);
				c.getItems().addItem(29001, 1);
				c.sendMessage("You upgrade your ancestral robe top to tier 2.");
			}

			if (useWith == 21024 && c.getItems().playerHasItem(21024) && c.getItems().playerHasItem(3461)) { //t2 bottoms creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(21024, 1);
				c.getItems().addItem(29002, 1);
				c.sendMessage("You upgrade your ancestral robe bottom to tier 2.");
			}

			if (useWith == 29000 && c.getItems().playerHasItem(29000) && c.getItems().playerHasItem(3461)) { //t3 hat creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29000, 1);
				c.getItems().addItem(29003, 1);
				c.sendMessage("You upgrade your ancestral hat to tier 3.");
			}

			if (useWith == 29001 && c.getItems().playerHasItem(29001) && c.getItems().playerHasItem(3461)) { //t3 body creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29001, 1);
				c.getItems().addItem(29004, 1);
				c.sendMessage("You upgrade your ancestral robe top to tier 3.");
			}

			if (useWith == 29002 && c.getItems().playerHasItem(29002) && c.getItems().playerHasItem(3461)) { //t3 bottoms creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29002, 1);
				c.getItems().addItem(29005, 1);
				c.sendMessage("You upgrade your ancestral robe bottom to tier 3.");
			}

			if (useWith == 29003 && c.getItems().playerHasItem(29003) && c.getItems().playerHasItem(3461)) { //t4 hat creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29003, 1);
				c.getItems().addItem(29006, 1);
				c.sendMessage("You upgrade your ancestral hat to tier 4.");
			}

			if (useWith == 29004 && c.getItems().playerHasItem(29004) && c.getItems().playerHasItem(3461)) { //t4 body creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29004, 1);
				c.getItems().addItem(29007, 1);
				c.sendMessage("You upgrade your ancestral robe top to tier 4.");
			}

			if (useWith == 29005 && c.getItems().playerHasItem(29005) && c.getItems().playerHasItem(3461)) { //t4 bottoms creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29005, 1);
				c.getItems().addItem(29008, 1);
				c.sendMessage("You upgrade your ancestral robe bottom to tier 4.");
			}

			if (useWith == 29006 && c.getItems().playerHasItem(29006) && c.getItems().playerHasItem(3461)) { //t5 hat creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29006, 1);
				c.getItems().addItem(29009, 1);
				c.sendMessage("You upgrade your ancestral hat to tier 5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Ancestral Hat to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Ancestral Hat to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
			...
			if (useWith == 29007 && c.getItems().playerHasItem(29007) && c.getItems().playerHasItem(3461)) { //t5 body creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29007, 1);
				c.getItems().addItem(29010, 1);
				c.sendMessage("You upgrade your ancestral robe top to tier 5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Ancestral Top to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Ancestral Top to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
			...
			if (useWith == 29008 && c.getItems().playerHasItem(29008) && c.getItems().playerHasItem(3461)) { //t5 bottoms creation
				c.getItems().deleteItem(3461, 1);
				c.getItems().deleteItem(29008, 1);
				c.getItems().addItem(29011, 1);
				c.sendMessage("You upgrade your ancestral robe bottom to tier 5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Ancestral Bottoms to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Ancestral Robe Bottoms to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 29046) { //sang staff

			if (useWith == 22323 && c.getItems().playerHasItem(22323) && c.getItems().playerHasItem(29046)) { //t2 sang
				c.getItems().deleteItem(29046, 1);
				c.getItems().deleteItem(22323, 1);
				c.getItems().addItem(3696, 1);
				c.sendMessage("You upgrade the sanguinesti staff to T2.");
			}

			if (useWith == 3696 && c.getItems().playerHasItem(3696) && c.getItems().playerHasItem(29046)) { //t3 sang
				c.getItems().deleteItem(29046, 1);
				c.getItems().deleteItem(3696, 1);
				c.getItems().addItem(29043, 1);
				c.sendMessage("You upgrade the sanguinesti staff to T3.");
			}

			if (useWith == 29043 && c.getItems().playerHasItem(29043) && c.getItems().playerHasItem(29046)) { //t4 sang
				c.getItems().deleteItem(29046, 1);
				c.getItems().deleteItem(29043, 1);
				c.getItems().addItem(29044, 1);
				c.sendMessage("You upgrade the sanguinesti staff to T4.");
			}

			if (useWith == 29044 && c.getItems().playerHasItem(29044) && c.getItems().playerHasItem(29046)) { //t5 sang
				c.getItems().deleteItem(29046, 1);
				c.getItems().deleteItem(29044, 1);
				c.getItems().addItem(29045, 1);
				c.sendMessage("You upgrade the sanguinesti staff to T5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Sanguinesti Staff to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Sanguinesti Staff to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 29042) { //avernic defender

			if (useWith == 22322 && c.getItems().playerHasItem(22322) && c.getItems().playerHasItem(29042)) { //t2 avernic
				c.getItems().deleteItem(29042, 1);
				c.getItems().deleteItem(22322, 1);
				c.getItems().addItem(29038, 1);
				c.sendMessage("You upgrade the avernic defender to T2.");
			}

			if (useWith == 29038 && c.getItems().playerHasItem(29038) && c.getItems().playerHasItem(29042)) { //t3 avernic
				c.getItems().deleteItem(29042, 1);
				c.getItems().deleteItem(29038, 1);
				c.getItems().addItem(29039, 1);
				c.sendMessage("You upgrade the avernic defender to T3.");
			}

			if (useWith == 29039 && c.getItems().playerHasItem(29039) && c.getItems().playerHasItem(29042)) { //t4 avernic
				c.getItems().deleteItem(29042, 1);
				c.getItems().deleteItem(29039, 1);
				c.getItems().addItem(29040, 1);
				c.sendMessage("You upgrade the avernic defender to T4.");
			}

			if (useWith == 29040 && c.getItems().playerHasItem(29040) && c.getItems().playerHasItem(29042)) { //t5 avernic
				c.getItems().deleteItem(29042, 1);
				c.getItems().deleteItem(29040, 1);
				c.getItems().addItem(29041, 1);
				c.sendMessage("You upgrade the avernic defender to T5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Avernic Defender to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Avernic Defender to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 29034) {
			...
			if (useWith == 13072 && c.getItems().playerHasItem(13072) && c.getItems().playerHasItem(29034)) { //t2 top
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(13072, 1);
				c.getItems().addItem(29027, 1);
				c.sendMessage("You upgrade the elite void top to T2.");
			}

			if (useWith == 13073 && c.getItems().playerHasItem(13073) && c.getItems().playerHasItem(29034)) { //t2 bottoms
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(13073, 1);
				c.getItems().addItem(29026, 1);
				c.sendMessage("You upgrade the elite void bottoms to T2.");
			}

			if (useWith == 29027 && c.getItems().playerHasItem(29027) && c.getItems().playerHasItem(29034)) { //t3 top
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(29027, 1);
				c.getItems().addItem(29029, 1);
				c.sendMessage("You upgrade the elite void top to T3.");
			}

			if (useWith == 29026 && c.getItems().playerHasItem(29026) && c.getItems().playerHasItem(29034)) { //t3 bottoms
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(29026, 1);
				c.getItems().addItem(29028, 1);
				c.sendMessage("You upgrade the elite void bottoms to T3.");
			}

			if (useWith == 29029 && c.getItems().playerHasItem(29029) && c.getItems().playerHasItem(29034)) { //t4 top
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(29029, 1);
				c.getItems().addItem(29031, 1);
				c.sendMessage("You upgrade the elite void top to T4.");
			}

			if (useWith == 29028 && c.getItems().playerHasItem(29028) && c.getItems().playerHasItem(29034)) { //t4 bottoms
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(29028, 1);
				c.getItems().addItem(29030, 1);
				c.sendMessage("You upgrade the elite void bottoms to T4.");
			}

			if (useWith == 29031 && c.getItems().playerHasItem(29031) && c.getItems().playerHasItem(29034)) { //t5 top
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(29031, 1);
				c.getItems().addItem(29033, 1);
				c.sendMessage("You upgrade the elite void top to T5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Elite Void Top to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Elite Void Top to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
			...
			if (useWith == 29030 && c.getItems().playerHasItem(29030) && c.getItems().playerHasItem(29034)) { //t5 bottoms
				c.getItems().deleteItem(29034, 1);
				c.getItems().deleteItem(29030, 1);
				c.getItems().addItem(29032, 1);
				c.sendMessage("You upgrade the elite void bottoms to T5.");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Elite Void Bottoms to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Elite Void Bottoms to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 29025) {
			...
			if (useWith == 22109 && c.getItems().playerHasItem(22109) && c.getItems().playerHasItem(29025)) { //t2
				c.getItems().deleteItem(29025, 1);
				c.getItems().deleteItem(22109, 1);
				c.getItems().addItem(29021, 1);
				c.sendMessage("You apply the upgrade scroll to the avas assembler");
			}

			if (useWith == 29021 && c.getItems().playerHasItem(29021) && c.getItems().playerHasItem(29025)) { //t3
				c.getItems().deleteItem(29025, 1);
				c.getItems().deleteItem(29021, 1);
				c.getItems().addItem(29022, 1);
				c.sendMessage("You apply the upgrade scroll to the avas assembler T2");
			}

			if (useWith == 29022 && c.getItems().playerHasItem(29022) && c.getItems().playerHasItem(29025)) { //t4
				c.getItems().deleteItem(29025, 1);
				c.getItems().deleteItem(29022, 1);
				c.getItems().addItem(29023, 1);
				c.sendMessage("You apply the upgrade scroll to the avas assembler T3");
			}

			if (useWith == 29023 && c.getItems().playerHasItem(29023) && c.getItems().playerHasItem(29025)) { //t4
				c.getItems().deleteItem(29025, 1);
				c.getItems().deleteItem(29023, 1);
				c.getItems().addItem(29024, 1);
				c.sendMessage("You apply the upgrade scroll to the avas assembler T4");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Ava's Assembler to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Ava's Assembler to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 29020) {
			...
			if (useWith == 25865 && c.getItems().playerHasItem(25865) && c.getItems().playerHasItem(29020)) { //t2
				c.getItems().deleteItem(29020, 1);
				c.getItems().deleteItem(25865, 1);
				c.getItems().addItem(25867, 1);
				c.sendMessage("You apply the upgrade scroll to the bow of faerdhinen");
			}

			if (useWith == 25867 && c.getItems().playerHasItem(25867) && c.getItems().playerHasItem(29020)) { //t3
				c.getItems().deleteItem(29020, 1);
				c.getItems().deleteItem(25867, 1);
				c.getItems().addItem(25896, 1);
				c.sendMessage("You apply the upgrade scroll to the bow of faerdhinen T2");
			}

			if (useWith == 25896 && c.getItems().playerHasItem(25896) && c.getItems().playerHasItem(29020)) { //t4
				c.getItems().deleteItem(29020, 1);
				c.getItems().deleteItem(25896, 1);
				c.getItems().addItem(25888, 1);
				c.sendMessage("You apply the upgrade scroll to the bow of faerdhinen T3");
			}

			if (useWith == 25888 && c.getItems().playerHasItem(25888) && c.getItems().playerHasItem(29020)) { //t5
				c.getItems().deleteItem(29020, 1);
				c.getItems().deleteItem(25888, 1);
				c.getItems().addItem(25886, 1);
				c.sendMessage("You apply the upgrade scroll to the bow of faerdhinen T4");
				PlayerHandler.dropMessage(c.getDisplayNameFormatted() + " has upgraded the Bow of Faerdhinen to T5!");
				DiscordWebhook webhook = new DiscordWebhook(Configuration.AchievementsWebhook);
				webhook.addEmbed(new DiscordWebhook.EmbedObject()
						.setTitle("Item Tier Completion")
						.setDescription(c.getDisplayName() + " has upgraded the Bow of Faerdhinen to T5.")
						.setColor(Color.RED));
				try {
					if (Configuration.DiscordEnabled)
						webhook.execute(); //Handle exception
		...
		if (itemUsed == 24670) {
			if (useWith == 21018 && c.getItems().playerHasItem(21018) && c.getItems().playerHasItem(24670)) { //hat
				c.getItems().deleteItem(24670, 1);
				c.getItems().deleteItem(21018, 1);
				c.getItems().addItem(24664, 1);
				c.sendMessage("You apply the twisted kit to the ancestral hat");
			}
			if (useWith == 21021 && c.getItems().playerHasItem(21021) && c.getItems().playerHasItem(24670)) { //body
				c.getItems().deleteItem(24670, 1);
				c.getItems().deleteItem(21021, 1);
				c.getItems().addItem(24666, 1);
				c.sendMessage("You apply the twisted kit to the ancestral robe top");
			}
			if (useWith == 21024 && c.getItems().playerHasItem(21024) && c.getItems().playerHasItem(24670)) { //legs
				c.getItems().deleteItem(24670, 1);
				c.getItems().deleteItem(21024, 1);
				c.getItems().addItem(24668, 1);
				c.sendMessage("You apply the twisted kit to the ancestral robe bottoms");
			}
		}

		if (itemUsed == 26717) { //bandos kit
			if (useWith == 11832 && c.getItems().playerHasItem(11832) && c.getItems().playerHasItem(26717)) {
				c.getItems().deleteItem(26717, 1);
				c.getItems().deleteItem(11832, 1);
				c.getItems().addItem(26718, 1);
				c.sendMessage("You apply the bandos kit to the chestplate");
			}
			if (useWith == 11834 && c.getItems().playerHasItem(11834) && c.getItems().playerHasItem(26717)) {
				c.getItems().deleteItem(26717, 1);
				c.getItems().deleteItem(11834, 1);
				c.getItems().addItem(26719, 1);
				c.sendMessage("You apply the bandos kit to the tassets");
			}
		}
		if (itemUsed == 26713) {
			if (useWith == 11826 && c.getItems().playerHasItem(11826) && c.getItems().playerHasItem(26713)) {
				c.getItems().deleteItem(26713, 1);
				c.getItems().deleteItem(11826, 1);
				c.getItems().addItem(26714, 1);
				c.sendMessage("You apply the armadyl kit to the helmet");
			}
			if (useWith == 11828 && c.getItems().playerHasItem(11828) && c.getItems().playerHasItem(26713)) {
				c.getItems().deleteItem(26713, 1);
				c.getItems().deleteItem(11828, 1);
				c.getItems().addItem(26715, 1);
				c.sendMessage("You apply the armadyl kit to the chestplate");
			}
			if (useWith == 11830 && c.getItems().playerHasItem(11830) && c.getItems().playerHasItem(26713)) {
				c.getItems().deleteItem(26713, 1);
				c.getItems().deleteItem(11830, 1);
				c.getItems().addItem(26716, 1);
				c.sendMessage("You apply the armadyl kit to the chainskirt");
			}
		}
		...
		if (itemUsed == 23804 && useWith == 8921) {
			c.getItems().deleteItem(useWith, 1);
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().addItem(11784, 1);
		}

		if (itemUsed == 23804 && useWith == 11864) {
			c.getItems().deleteItem(useWith, 1);
			c.getItems().deleteItem(itemUsed, 1);
			c.getItems().addItem(11865, 1);
		}

		if(c.getItems().playerHasItem(4551) && c.getItems().playerHasItem(4166) && c.getItems().playerHasItem(4168)
		&& c.getItems().playerHasItem(4164) && c.getItems().playerHasItem(8901) &&c.getItems().playerHasItem(4155)){
			...
			c.getDH().sendItemStatement("You sucessfully make the slayer helmet", 11864);
		}


		if(c.getItems().playerHasItem(4551) && c.getItems().playerHasItem(4166) && c.getItems().playerHasItem(4168)
				&& c.getItems().playerHasItem(4164) && c.getItems().playerHasItem(11784) &&c.getItems().playerHasItem(4155)){
			c.getItems().deleteItem2(4551, 1);
			c.getItems().deleteItem2(4166, 1);
			c.getItems().deleteItem2(4168, 1);
			c.getItems().deleteItem2(4164, 1);
			...
			c.getDH().sendItemStatement("You sucessfully make the slayer helmet (I)", 11865);
		}
		...
		if (itemUsed == 23804 && !(useWith == 6737 || useWith == 6731 || useWith == 6733 || useWith == 12601 || useWith == 12603
				|| useWith == 12605 || useWith == 8921 || useWith == 11864)) {
			c.sendMessage("This can only be used on dagganoth rings slayer helmet black mask and wildy rings.");
			return;
		}
		...
		if (itemUsed == 554 || itemUsed == 560 || itemUsed == 562) {
			if (useWith == 11907)
				c.start(new DialogueBuilder(c).option("Charge trident?",
						new DialogueOption("Yes.", player -> {
							player.getCT().chargeTrident();
							player.getPA().closeAllWindows();
		...
		if (itemUsed == 554 || itemUsed == 560 || itemUsed == 562 || itemUsed == 12934) {
			if (useWith == 12899)
				c.start(new DialogueBuilder(c).option("Charge trident?",
						new DialogueOption("Yes.", player -> {
							player.getCT().chargeToxicTrident();
							player.getPA().closeAllWindows();
		...
		//Ancient godsword
		if (itemUsed == 11798 && useWith == 26370 || itemUsed == 26370 && useWith == 11798) {
			c.getItems().deleteItem(26370, c.getItems().getInventoryItemSlot(26370), 1);
			c.getItems().deleteItem(11798, c.getItems().getInventoryItemSlot(11798), 1);
			c.getItems().addItem(26233, 1);
			c.getDH().sendStatement("You combine the hilt and the blade to make a full godsword.");
		}

		//Zaryte crossbow
		if (itemUsed == Items.ARMADYL_CROSSBOW && useWith == 26372 || itemUsed == 26372 && useWith == Items.ARMADYL_CROSSBOW) {
			if (!c.getItems().playerHasItem(26231, 250)) {
				c.sendMessage("You need 250 Nihil shards to make this.");
				return;
			}
			c.getItems().deleteItem(26372, c.getItems().getInventoryItemSlot(26372), 1);
			c.getItems().deleteItem(Items.ARMADYL_CROSSBOW, c.getItems().getInventoryItemSlot(Items.ARMADYL_CROSSBOW), 1);
			c.getItems().deleteItem(26231, 250);
			c.getItems().addItem(26374, 1);
			c.getDH().sendStatement("You combine the Armadyl crossbow, Nihil shards and Nihil horn,", "resuling in a Zaryte crossbow.");
		}
	...
	public static void handleItemOnNPC(Player player, NPC npc, int itemId, int slot) {
		...
		if (npcId == 7690) {
			if (itemId == 21295 && player.getItems().playerHasItem(21295)) {
				player.getItems().deleteItem(21295, 1);
			}
			if (Misc.hasOneOutOf(15)) {
				player.getItems().addItemUnderAnyCircumstance(22319, 1);
			//	player.sendMessage("@gre@You lucky, take care of TzRek Zuk");
				PlayerHandler.executeGlobalMessage("@cr35@ @red@[INFERNO] " + player.getDisplayName() + " received a tzRek Zuk from gambling their infernal cape.");
			} else {
				player.sendMessage("@red@You not lucky.");
			}
		}
