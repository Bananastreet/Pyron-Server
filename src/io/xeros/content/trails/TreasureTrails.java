...
public class TreasureTrails {
	...
	private static void announceRare(Player player, GameItem item, RewardLevel difficulty) {
		PlayerHandler.dropMessage(player.getDisplayNameFormatted() + " received " + ItemDef.forId(item.getId()).getName() + " from " +
				difficulty.name().toLowerCase() + " clue #" + getCluesCompleted(player, difficulty) + "!");
	}

	public static boolean rollMimicCasket(RewardLevel rewardLevel) {
		int roll = rewardLevel == RewardLevel.MASTER ? 5
				: rewardLevel == RewardLevel.HARD ? 15 : 0;
		...
		switch (rewardLevel) {
			case EASY:
				...
				if (Configuration.leaderboardEnabled) {
					LeaderboardSerialisation.INSTANCE.updateCount(player, LeaderboardData.MISCELLANEOUS,
							player.getEasyClueCounter(), io.xeros.content.leaderboard.impl.Misc.EASY_CLUES);
				}
				break;
			case MEDIUM:
				...
				if (Configuration.leaderboardEnabled) {
					LeaderboardSerialisation.INSTANCE.updateCount(player, LeaderboardData.MISCELLANEOUS,
							player.getMediumClueCounter(), io.xeros.content.leaderboard.impl.Misc.MEDIUM_CLUES);
				}
				break;
			case HARD:
				...
				if (Configuration.leaderboardEnabled) {
					LeaderboardSerialisation.INSTANCE.updateCount(player, LeaderboardData.MISCELLANEOUS,
							player.getHardClueCounter(), io.xeros.content.leaderboard.impl.Misc.HARD_CLUES);
				}
				break;
			case MASTER:
				...
				if (Configuration.leaderboardEnabled) {
					LeaderboardSerialisation.INSTANCE.updateCount(player, LeaderboardData.MISCELLANEOUS,
							player.getMasterClueCounter(), io.xeros.content.leaderboard.impl.Misc.MASTER_CLUES);
				}
				break;
		}
	}
