...
public class Hunllef extends LegacySoloPlayerInstance {
	...
	public static void rewardPlayers(Player player) {
		if (Boundary.isIn(player, Boundary.HUNLLEF_CAVE)) {
			...
			player.getPA().spellTeleport(3029, 6124, 1, false);
			int keyAmount = 1;
			if (player.hasPerk(Player.HUNLLEF_DOUBLER) && Misc.hasOneOutOf(4)) {
				keyAmount = 2;
			} else {
				keyAmount = 1;
			}
			player.getItems().addItemUnderAnyCircumstance(23776, keyAmount);
			player.sendMessage("@red@Use the key on the reward chest.");
