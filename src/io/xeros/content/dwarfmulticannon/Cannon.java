...
public class Cannon {
    ...
    public static void attemptPlace(Player player) {
        ...
        } else if (Boundary.SEREN_ROOM.in(player)) {
            player.sendMessage("You can not place a cannon at seren.");
        } else if (Boundary.TARN_AREA.in(player)) {
            player.sendMessage("You can not place a cannon at mutant tarn.");
        } else if (Boundary.VOTE_BOSS.in(player)) {
            player.sendMessage("You can not place a cannon at vote boss.");
        } else if (Boundary.ABOMINATION.in(player)) {
            player.sendMessage("You can not place a cannon at abomination.");
        } else if (Boundary.AVATAR_OF_DESTRUCTION.in(player)) {
            player.sendMessage("You can not place a cannon at avatar of destruction.");
        } else if (Boundary.INADEQUACY.in(player)) {
            player.sendMessage("You can not place a cannon at the inadequacy.");
            ...
            if (cannonballsLoaded > 0) {
                for (NPC npc : npcs) {
                    ...
                    if (!player.usingGraniteCannonballs) {
                        ...
                        Projectile.createTargeted(getPosition().getCenterPosition(3), 2, npc, new ProjectileBaseBuilder().setProjectileId(1443).setCurve(0).setSendDelay(2).createProjectileBase()).send(null);
    ...
    private int getCannonMaxAmmoCount(Player player) {
        return DonatorRank.determineCannonAmmo(DonatorRank.getHighestRank(player), player);
    }
}
