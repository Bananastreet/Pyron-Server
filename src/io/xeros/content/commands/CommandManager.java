...
public class CommandManager {
    ...
    public static final List<CommandPackage> COMMAND_PACKAGES = Lists.newArrayList(new CommandPackage("admin", Right.ADMINISTRATOR), new CommandPackage("owner", Right.OWNER), new CommandPackage("moderator", Right.MODERATOR), new CommandPackage("helper", Right.HELPER), new CommandPackage("donator", Right.SAPPHIRE_DONATOR), new CommandPackage("all", Right.PLAYER));

    private static boolean hasRightsRequirement(Player c, Right rightsRequired) {
        if (rightsRequired == Right.SAPPHIRE_DONATOR && c.getRights().hasStaffPosition()) {
            return true;
        }
