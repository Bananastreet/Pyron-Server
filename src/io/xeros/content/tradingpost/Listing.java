...
public class Listing {
	...
	public static void buyListing(Player c, int slot, int amount) {
		...
		if (1 > amount) {
			c.sendMessage("You can not buy an item below quantity 1");
			return;
		}
	...
	public static void postButtons(Player c, int button) {
		switch(button) {
			case 189237:
				...
				for(Sale sale : sales)
					total++;
				if(c.tokensSpent <= 499 && total >= 6) {
					c.sendMessage("[@red@Trading Post@bla@] You cannot have more then 6 listings as a regular player.");
					return;
				} else if(c.tokensSpent >= 500 && c.tokensSpent <= 2499 && total >= 7) { //regular donator
					c.sendMessage("[@red@Trading Post@bla@] You cannot have more then 7 listings at your donator rank.");
					return;
				} else if(c.tokensSpent >= 2500 && c.tokensSpent <= 4999 && total >= 8) { //extreme donator
					c.sendMessage("[@red@Trading Post@bla@] You cannot have more then 8 listings at your donator rank.");
					return;
				} else if(c.tokensSpent >= 5000 && c.tokensSpent <= 9999 && total >= 9) { //legendary donator
					c.sendMessage("[@red@Trading Post@bla@] You cannot have more then 9 listings at your donator rank.");
					return;
				} else if(c.tokensSpent >= 10000 && c.tokensSpent <= 25000 && total >= 10) { //diamond club
					c.sendMessage("[@red@Trading Post@bla@] You cannot have more then 10 listings at your donator rank.");
					return;
				} else if(c.tokensSpent >= 50000 && total >= 11) { //onyx club
					c.sendMessage("[@red@Trading Post@bla@] You cannot have more then 11 listings at your donator rank.");
					return;
				}
				if(!c.inSelecting) {
					openNewListing(c);
