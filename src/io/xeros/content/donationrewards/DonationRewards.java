...
public class DonationRewards {
    ...
    public void openInterface() {
        ...
        player.getPA().sendString(AMOUNT_DONATED_TEXT_ID, "" + amountDonatedThisWeek + " / " + lastItemPrice + " Tokens");
        ...
        if (amount > 0) {
            ...
            for (DonationReward reward : DonationReward.getRewardList()) {
                if (reward.getPrice() > amountDonatedThisWeek && reward.getPrice() <= amountDonatedThisWeek + amount) {
                    player.getInventory().addToBank(new ImmutableItem(reward.getItem().getId(), reward.getItem().getAmount()));
                   // player.getInventory().addToInventory(new ImmutableItem(reward.getItem().getId(), reward.getItem().getAmount()));
                    player.sendMessage(TEXT_COLOUR + "You have received " + reward.getItem().getAmount() + " x " + ItemDef.forId(reward.getItem().getId()).getName() + " from donation rewards!");
                    player.sendMessage(TEXT_COLOUR + "Your donation reward has been sent to your bank.");
                    gainedItem = true;
            ...
            if (amountDonatedThisWeek >= 5000) {
                amountDonatedThisWeek = 0;
                player.sendMessage("@red@Your donation reward progress has been reset.");
            }
        }
    }
