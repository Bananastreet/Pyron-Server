...
//			}
	...
	public void handleSuperiorSpawn() {
		task.ifPresent(task -> {
			int chance = player.trinketOfSuperiorityActive ? Misc.random(38) : Misc.random(75);
			if (chance == 1 || Server.isDebug()) {
				if (superiorSpawned){
					return;
				}
	...
	public int getCancelTaskCost() {
		return DonatorRank.determineCancelCost(DonatorRank.getHighestRank(player));
	}
	...
	public void removeTask() {
		...
		for (int index = 0; index < removed.length; index++) {
			if (index > 2 && !player.getRights().isOrInherits(Right.SAPPHIRE_DONATOR)) {
	...
	public int getBlockTaskCost() {
		return DonatorRank.determineBlockCost(DonatorRank.getHighestRank(player));
	}
