 ...
 */
	...
	public void steal(Pickpocket pickpocket, NPC npc) {
		...
		switch (pickpocket) {
			case VYRE:
				int chance = 1500;
				if (player.hasPerk(Player.BLOOD_STEALER)) {
					chance -= 950;
				}
				if (Boundary.isIn(player, Boundary.RUBY_VYRE)) {
					chance -= 250;
				}
				if (Misc.hasOneOutOf(chance)) {
					player.getItems().addItem(24777, 1);
					PlayerHandler.executeGlobalMessage("@cr35@ @blu@[SKILLING] @red@" + player.getDisplayName() + " has received a blood shard from thieving!");
				}
				break;
		case FARMER:
			if (Boundary.isIn(player, Boundary.ARDOUGNE_BOUNDARY)) {
	...
	private enum Rarity {
		ALWAYS(0), COMMON(5), UNCOMMON(10), RARE(15), VERY_RARE(25), SUPER_RARE(5);
	...
	public enum Pickpocket {
		...
		FARMER(38, 80, 20000, new HashMap<Rarity, List<GameItem>>() {
			{
		...
		}), VYRE(85, 110, 12000, new HashMap<Rarity, List<GameItem>>() {
			{
