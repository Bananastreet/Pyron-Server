...
public class MiningEvent extends Event<Player> {
	...
	public void update() {
		...
		if (RandomUtils.nextInt(1, 300) == 1 && attachment.getInterfaceEvent().isExecutable() && mineral != Mineral.SHOOTING_STAR) {
	...
	public void execute() {
		...
		case CLAY:
			...
			break;
		case SHOOTING_STAR:
			ShootingStar.updateStar("dust");
			if (!ShootingStar.active) { stop(); }
			break;
		default:
			break;
		...
		if (attachment.hasPerk(Player.MIGHTY_MINER) && Misc.hasOneOutOf(2)) {
			amount *= 2;
		}
		...
		if (!Boundary.isIn(attachment, Boundary.HESPORI)) {
			if (mineral == Mineral.GEM) {
				if (itemId == 6571) {
					PlayerHandler.dropMessage(attachment.getDisplayNameFormatted() + " received a drop: " + amount + " x " + ItemDef.forId(itemId).getName() +
							" from a gem rock!");
				}
			}
		}
		...
		if (Misc.random(mineral.getPetChance() / dropRate) == 10 && !Boundary.isIn(attachment, Boundary.HESPORI)) {
		...
		if (Misc.random(petRate) == 2 && attachment.getItems().getItemCount(13321, false) == 0
				&& attachment.petSummonId != 7439 && !Boundary.isIn(attachment, Boundary.HESPORI)) {
