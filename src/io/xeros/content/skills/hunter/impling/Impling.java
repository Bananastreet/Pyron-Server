...
public class Impling {
	...
	public static void catchImpling(Player player, NPC npc) {
		...
		if (!(player.hasRank(Right.SAPPHIRE_DONATOR)) && !bareHanded && !player.getItems().playerHasItem(10010) && !player.getItems().isWearingItem(10010)) {
		...
		if (bareHanded) {
			if (!player.getItems().playerHasItem(10010) && !player.getItems().isWearingItem(10010)) {
				if (!(player.hasRank(Right.SAPPHIRE_DONATOR))) {
	...
	public enum ImpRewards {
		...
		LUCKY(19732, new int[][] {
				{995,2000000}, {6585, 1}, {1514, 500}, {4088, 3}, {4586, 3}, {11730, 1}, {2364, 25},
				{2362, 100}, {11937, 500}, {537, 25},  {10926, 125}, {12696, 50}, {11959, 200}
		}),
		...
		public static void getReward(Player c, int itemId) {
			...
			if(t.getRewards()[r].length == 3) {
				...
				c.getItems().addItemUnderAnyCircumstance(t.getRewards()[r][0], amount);
				c.sendMessage("x" + amount + " " + ItemAssistant.getItemName(t.getRewards()[r][0]));
			} else {
				c.getItems().addItemUnderAnyCircumstance(t.getRewards()[r][0], t.getRewards()[r][1]);
