...
public class Smelting {
		...
		if (c.smeltAmount > 0) {
			...
			if (hasItems) {
				...
				if (c.bar.twoOres()) {
					...
					var ore2IsCoal = c.bar.getOre2() == 453;
					if (!(ore2IsCoal && c.hasPerk(Player.MASTER_SMITHER) && Misc.hasOneOutOf(2))) {
					...
					c.getItems().deleteItem(c.bar.getOre1(), 1);
					if ((c.bar.getBar() == 2351 && Misc.random(100) >= 50) && c.playerEquipment[Player.playerRing] != 2568 && !c.hasPerk(Player.MASTER_SMITHER))
	...
	private static boolean hasItems(Player c, int firstOre, int secondOre, int createdBar) {
		...
		if (secondOre > 0) {
			if (c.getItems().playerHasItem(firstOre) && c.getItems().playerHasItem(secondOre) || c.getItems().playerHasItem(firstOre) && c.getItems().playerHasItem(12019))
				return true;
		} else {
