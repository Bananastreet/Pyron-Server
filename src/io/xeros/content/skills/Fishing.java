				...
				Server.getEventHandler().submit(new Event<Player>("skilling", player, getTimer(player, npcId) + 5 + playerFishingLevel(player)) {
					...
					public void execute() {
						...
						if (player.playerSkillProp[10][1] > 0) {
							...
							int multiplier = 1;
							if (player.hasPerk(Player.RAW_COLLECTOR)) {
								multiplier = 3;
							}
							player.getItems().addItem(player.playerSkillProp[10][1], SkillcapePerks.FISHING.isWearing(player) || SkillcapePerks.isWearingMaxCape(player) ? 2 * multiplier : 1 * multiplier);
							player.startAnimation(player.playerSkillProp[10][0]);
